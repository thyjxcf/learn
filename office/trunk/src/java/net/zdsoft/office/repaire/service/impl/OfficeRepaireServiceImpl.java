...
public class OfficeRepaireServiceImpl implements OfficeRepaireService{
	...
	private WeikePushService weikePushService;
	...
	public void add(OfficeRepaire officeRepaire,UploadFile file){//TODO
		...
		//发送维修提醒消息时绑定跳转信息
		officeBusinessJumpService.pushMsgUrl(re, OfficeBusinessJump.OFFICE_REPAIRE, officeMsgSending.getId());
		//推送消息到微课
		officeBusinessJumpService.pushMsgUrlToMobile(re, OfficeBusinessJump.OFFICE_REPAIRE, userIdsSet.toArray(new String[0]), WeikeAppConstant.AUDIT_URL);
	...
	public void updateState(String[] ids, String state,String userId) {
		officeRepaireDao.updateState(ids, state,userId);
	}
	@Override
	public void update(OfficeRepaire officeRepaire,UploadFile file){//TODO
		...
		//发送维修提醒消息时绑定跳转信息
		officeBusinessJumpService.pushMsgUrl(officeRepaire, OfficeBusinessJump.OFFICE_REPAIRE, officeMsgSending.getId());
		//推送消息到微课
		officeBusinessJumpService.pushMsgUrlToMobile(officeRepaire, OfficeBusinessJump.OFFICE_REPAIRE, userIdsSet.toArray(new String[0]), WeikeAppConstant.AUDIT_URL);
	...
	public void updateState(OfficeRepaire officeRepaire) {//TODO
		if(StringUtils.equals(officeRepaire.getState(), "3")){//当维修完毕，则发送消息给报修人
			...
			//推送消息到微课
			officeBusinessJumpService.pushMsgUrlToMobile(officeRepaire, OfficeBusinessJump.OFFICE_REPAIRE, new String[]{officeRepaire.getUserId()}, WeikeAppConstant.DETAILE_URL);
	...
	public Map<String, String> getOfficeRepaireByUnitIdAndYearsMap(
		...
		return officeRepaireDao.getOfficeRepaireByUnitIdAndYearsMap(unitId, years, change);
	}
	
	@Override
	public Map<String, String> getOfficeRepaireByUnitIdAndYearsTalMap(
			String unitId, String years, String change) {
		return officeRepaireDao.getOfficeRepaireByUnitIdAndYearsTalMap(unitId, years, change);
	}
	
	@Override
	public Map<String, String> getOfficeRepaireByUnitIdAndYearsCountMap(
			String unitId, String years) {
		return officeRepaireDao.getOfficeRepaireByUnitIdAndYearsCountMap(unitId, years);
	}
	@Override
	public OfficeRepaire getOfficeRepaireById(String id){
		...
		User user = userService.getUser(officeRepaire.getUserId());
		officeRepaire.setUserName(user.getRealname());
		if(StringUtils.isNotBlank(officeRepaire.getRepaireUserId())){
			User user2=userService.getUser(officeRepaire.getRepaireUserId());
			officeRepaire.setRepaireUserName(user2.getRealname());
		}
	...
	public void setWeikePushService(WeikePushService weikePushService) {
		this.weikePushService = weikePushService;
	}
