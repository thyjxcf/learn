...
//				and END_TIME <= to_date('2015-10-30 14:26','yyyy-MM-dd HH24:mi')))
		sql.append(" and ((CALENDAR_TIME<= to_date(?,'yyyy-MM-dd HH24:mi') and END_TIME>=to_date(?,'yyyy-MM-dd HH24:mi'))");
		sql.append(" or (CALENDAR_TIME<=to_date(?,'yyyy-MM-dd HH24:mi') and END_TIME >=to_date(?,'yyyy-MM-dd HH24:mi'))");
		sql.append(" or (CALENDAR_TIME >= to_date(?,'yyyy-MM-dd HH24:mi') and END_TIME <= to_date(?,'yyyy-MM-dd HH24:mi')))");
	...
	public List<OfficeCalendar> getOfficeCalendarListPage(String unitId,
		...
		if(StringUtils.isNotBlank(startTime)){
			sql.append(" and to_date(?, 'yyyy-MM-dd') <= to_date(to_char(calendar_time, 'yyyy-MM-dd'), 'yyyy-MM-dd')");
			args.add(startTime);
		}
	...
	@Override
	public List<OfficeCalendar> getCalendarListByConditions(String unitId, String[] userIds, String creatorType, 
			String startTime, String endTime, Pagination page){
		List<Object> argsList = new ArrayList<Object>();
		StringBuffer sql = new StringBuffer("select * from office_calendar where unit_id = ? ");
		argsList.add(unitId);
		if(StringUtils.isNotBlank(creatorType)){
			sql.append(" and creator_type = ? ");
			argsList.add(creatorType);
		}
		if(StringUtils.isNotBlank(startTime)){
			sql.append(" and to_date(?, 'yyyy-MM-dd') >= to_date(to_char(calendar_time, 'yyyy-MM-dd'), 'yyyy-MM-dd')");
			sql.append(" and to_date(?, 'yyyy-MM-dd') <= to_date(to_char(end_time, 'yyyy-MM-dd'), 'yyyy-MM-dd')");
			argsList.add(startTime);
			argsList.add(startTime);
		}
		if(StringUtils.isNotBlank(endTime)){
