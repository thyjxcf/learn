...
public class OfficeWorkOutlineServiceImpl implements OfficeWorkOutlineService{
	...
	@Override
	public OfficeWorkOutline save(OfficeWorkOutline officeWorkOutline, UploadFile file){
		sendMsg(officeWorkOutline);
		officeWorkOutline = officeWorkOutlineDao.save(officeWorkOutline);
		if(file!=null){
			Attachment attachment = new Attachment();
			attachment.setFileName(file.getFileName());
			attachment.setContentType(file.getContentType());
			attachment.setFileSize(file.getFileSize());
			attachment.setUnitId(officeWorkOutline.getUnitId());
			attachment.setObjectId(officeWorkOutline.getId());
	...
	@Override
	public Integer update(OfficeWorkOutline officeWorkOutline, UploadFile file){
		sendMsg(officeWorkOutline);
		Integer num = officeWorkOutlineDao.update(officeWorkOutline);
		
		List<Attachment> attachments = attachmentService.getAttachments(officeWorkOutline.getId(), Constants.OFFICE_BUSINESS_TRIP_ATT);
		if(file!=null){
		...
		if(StringUtils.isBlank(officeWorkOutline.getUploadContentFileInput())&&file==null){
	...
	public OfficeWorkOutline getOfficeWorkOutlineById(String id){
		OfficeWorkOutline officeWorkOutline = officeWorkOutlineDao.getOfficeWorkOutlineById(id);
		officeWorkOutline.setAttachments(attachmentService.getAttachments(officeWorkOutline.getId(), Constants.OFFICE_SCHEDULE_AIT));
		return officeWorkOutline;
	}
