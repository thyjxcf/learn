...
public class OfficeSealServiceImpl implements OfficeSealService{
	...
	public OfficeSeal save(OfficeSeal officeSeal) {
		OfficeSeal officeSeals=officeSealDao.save(officeSeal);
	...
	public void save(OfficeSeal officeSeal,UploadFile file){
		//发送到消息中心
		officeSealDao.save(officeSeal);
		if(file!=null){
			Attachment attachment = new Attachment();
			attachment.setFileName(file.getFileName());
			attachment.setContentType(file.getContentType());
			attachment.setFileSize(file.getFileSize());
			attachment.setUnitId(officeSeal.getUnitId());
			attachment.setObjectId(officeSeal.getId());
	...
	public Integer delete(String[] ids){
		List<Attachment> attachments = attachmentService.getAttachments(ids[0], Constants.OFFICE_SEAL_AIT);
	...
	public void update(OfficeSeal officeSeal,UploadFile file){
		...
		List<Attachment> attachments = attachmentService.getAttachments(officeSeal.getId(), Constants.OFFICE_SEAL_AIT);
		if(file!=null){
			if(CollectionUtils.isNotEmpty(attachments)){
		...
		if(StringUtils.isBlank(officeSeal.getUploadContentFileInput())&&file==null&&!officeSeal.isDelete()){
			String[] attachmentIds = new String[attachments.size()];
			for(int i = 0; i < attachments.size(); i++){
				attachmentIds[i] = attachments.get(i).getId();
			}
		...
		officeSealDao.update(officeSeal);
	}
	...
	public void remindMessage(OfficeSeal ent){
		...
		if(StringUtils.equals("2", ent.getState())){
			...
			if(StringUtils.isNotBlank(userId)){
				...
				officeBusinessJumpService.pushMsgUrl(ent, msgType, msgId);
			}
			...
			//获取审核人信息
			Unit unit = unitService.getUnit(user.getUnitid());
			String msgId=officeSubsystemService.sendMsgDetail(user, unit, title, content, content, false, msgType, userManageIds.toArray(new String[0]), null);
			officeBusinessJumpService.pushMsgUrl(ent, msgType, msgId);
	...
	public OfficeSeal getOfficeSealById(String id){//TODO
		...
		if(officeSeal!=null){
			...
			List<Attachment> attachments=attachmentService.getAttachments(officeSeal.getId(), Constants.OFFICE_SEAL_AIT);
			if(CollectionUtils.isNotEmpty(attachments)){
				officeSeal.setAttachments(attachments);
			}
		}
		return officeSeal;
	}
