...
public class OfficeCarApplyDaoImpl extends BaseDao<OfficeCarApply> implements OfficeCarApplyDao{
	...
	public Integer update(OfficeCarApply officeCarApply){
		String sql = "update office_car_apply set link_user_id = ?, mobile_phone = ?, person_number = ?, reason = ?, car_location = ?, is_goback = ?, is_need_waiting = ?, waiting_time = ?, use_time = ?, state = ?,car_id=? where id = ?";
		Object[] args = new Object[]{
			...
			officeCarApply.getUseTime(), officeCarApply.getState(),officeCarApply.getCarId(), 
			officeCarApply.getId()
		};
	...
	public List<OfficeCarApply> getOfficeCarApplyByUnitIdPage(String unitId,
		...
		if(StringUtils.isNotBlank(startTime)){
			sql.append(" and to_date(?, 'yyyy-MM-dd') <= to_date(to_char(use_time, 'yyyy-MM-dd'), 'yyyy-MM-dd')");
			objs.add(startTime);
		}
