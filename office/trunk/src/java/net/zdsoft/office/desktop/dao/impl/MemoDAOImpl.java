...
public class MemoDAOImpl extends BasicDAO implements MemoDAO {
    ...
    private static final String SQL_FIND_MEMOS_CPUNT = "select count(1) count,to_char( om.creation_time,'yyyy-mm-dd') datestr from OFFICE_MEMO om,office_memo_ex omx";
    private static final String SQL_FIND_MEMOS_CPUNT_IN_DATE = "select count(*) count from OFFICE_MEMO om,office_memo_ex omx";
    ...
    private static final String SQL_FIND_MEMO_BY_USERID = "SELECT om.*,omx.is_send,omx.id memoExId send FROM office_memo om,office_memo_ex omx"
            + " WHERE om.id=omx.memo_id and omx.use_user_id=? ORDER BY om.creation_time DESC";
    ...
    private static final String SQL_FIND_MEMO = "SELECT om.*,omx.is_send send,omx.id memoExId FROM office_memo om,office_memo_ex omx";
    
    private static final String SQL_FIND_MEMO_LIST_BY_USERID ="select om.*,omx.is_send send,omx.id memoExId from OFFICE_MEMO om,office_memo_ex omx where omx.use_user_id=? and om.id=omx.memo_id and om.creation_time-to_date(to_char(sysdate, 'yyyy-MM-dd'), 'yyyy-MM-dd')>=0 order by om.creation_time";
    ...
    public List<Memo> findMemosByTime(String userId, Date startDate, Date endDate, Pagination page, boolean isDesc) {
        ...
        sql.and("omx.use_user_id=? and om.id=omx.memo_id", userId, !Validators.isBlank(userId));
    ...
    private static class MemoMultiRowMapper implements MultiRowMapper {
        ...
        public Object mapRow(ResultSet rs, int index) throws SQLException {
            ...
            if(this.isExistColumn(rs,"send")){
            	memo.setSend(rs.getString("send"));
            }
            if(this.isExistColumn(rs, "memoExId")){
            	memo.setMemoExId(rs.getString("memoExId"));
            }
            return memo;
        }
    ...
    public List<Memo> findMemosByDayDesc(String userId, String[] dateStrs, Pagination page) {
        ...
        SQLCreater sql = new SQLCreater(SQL_FIND_MEMO, false);
        sql.andIn("to_char(om.creation_time,'yyyy-mm-dd') and om.id=omx.memo_id", dateStrs, Types.VARCHAR, true);
        sql.and("omx.use_user_id= ?", userId, Types.VARCHAR, true);
        sql.orderBy("om.creation_time", true);
        if (null != page) {
            return query(sql.getSQL(), sql.getArgs(), new MemoMultiRowMapper(), page);
        }
        else {
    ...
    public List<Memo> findMemosByDayAsc(String userId, String[] dateStrs, Pagination page) {
        ...
        sql.andIn("to_char(om.creation_time,'yyyy-mm-dd') and om.id=omx.memo_id", dateStrs, Types.VARCHAR, true);
        sql.and("omx.use_user_id= ?", userId, Types.VARCHAR, true);
        sql.orderBy("om.creation_time", false);
        if (null != page) {
            return query(sql.getSQL(), sql.getArgs(), new MemoMultiRowMapper(), page);
        }
        else {
    ...
    public Map<String, Integer> findMenosCount(String userId, String[] dateStrs) {
        ...
        sql.andIn("to_char(om.creation_time,'yyyy-mm-dd')", dateStrs, Types.VARCHAR, true);
        sql.and("omx.use_user_id= ? and om.id=omx.memo_id", userId, Types.VARCHAR, true);
        String sqlStr = sql.getSQL() + " group by to_char( om.creation_time,'yyyy-mm-dd')";
    ...
    public Map<String, Integer> findMenosCount(String userId, Date startDate, Date endDate) {
        SQLCreater sql = new SQLCreater(SQL_FIND_MEMOS_CPUNT, false);
        sql.and("omx.use_user_id=? and om.id=omx.memo_id", userId, !Validators.isBlank(userId));
        sql.and("om.creation_time<=?", endDate, true);
        sql.and("om.creation_time>=?", startDate, true);
        String sqlStr = sql.getSQL() + " group by to_char( om.creation_time,'yyyy-mm-dd')";
    ...
    public int findMemoCount(String userId, Date date) {
        ...
        sql.and("omx.use_user_id=? and om.id=omx.memo_id", userId, true);
        sql.and("om.creation_time<=?", DateUtils.getEndDate(date), true);
        sql.and("om.creation_time>=?", DateUtils.getStartDate(date), true);
