...
//    private List<CalendarDayInfo> calendars;
    ...
    private boolean isSendMemo=false;
...
//        responseJSON(json);
    ...
    public String saveOrUpdate() {
        ...
        memoService.modifyMemo(memo, getUserId(), memo.getUserIds());
        return SUCCESS;
    }
    ...
    public String saveMemo() {
    	...
    	memo.setContent((String)json.get("contents"));
        ...
        String sendPersons="";
        //String sendPersonss="";
        //String sendPersonsss="";
        if(json.containsKey("userIds")){
        	sendPersons=json.getString("userIds");
        }
        if(json.containsKey("userIdss")){
        	sendPersons=json.getString("userIdss");
        }
        if(json.containsKey("userIdsss")){
        	sendPersons=json.getString("userIdsss");
        }
//        if(org.apache.commons.lang.StringUtils.isNotBlank(sendPersonss)){
//        	sendPersons=sendPersonss;
//        }else if(org.apache.commons.lang.StringUtils.isNotBlank(sendPersonsss)){
//        	sendPersons=sendPersonsss;
//        }
        
        
        try {
        	memoService.modifyMemo(memo, getUserId(), sendPersons);
		} catch (Exception e) {
			e.printStackTrace();
			jsonError="操作失败!";
		}
    ...
    public String addMon() {
        if (!Validators.isEmpty(memoId)) {
            ...
            if (null != memo) {
                ...
                date = memo.getTime();
                if(org.apache.commons.lang.StringUtils.equals(memo.getUserId(), getUserId())){
                	memo.setSend("0");
                }else{
                	memo.setSend("1");
                }
...
//        }
    ...
    public String removeMemos() {
        if (!Validators.isEmpty(ids)) {
            officeMemoExService.delete(ids);
...
//    }
	...
	public boolean isSendMemo() {
		CustomRole customRole=customRoleService.getCustomRoleByRoleCode(getUnitId(), "office_send_memo");
		if(customRole==null){
			return false;
		}
		List<CustomRoleUser> customRoleUsers=customRoleUserService.getCustomRoleUserListByUserId(getUserId());
	...
	public void setSendMemo(boolean isSendMemo) {
		this.isSendMemo = isSendMemo;
	}
