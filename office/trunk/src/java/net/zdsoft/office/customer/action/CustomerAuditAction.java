...
/*	private Date startTime;
	...
	public String auditList(){
		...
		Date endTime=searchCustomer.getEndTime();
		if(endTime!=null){
			Calendar c = Calendar.getInstance();
			c.setTime(endTime);
			c.add(Calendar.DAY_OF_YEAR,1);
			endTime=c.getTime();
			searchCustomer.setEndTime(endTime);
		}
		//startTime,endTime,applyUserName,name,
		customerApplyList = officeCustomerApplyService.getCustomerApplyAuditList(customer, 
				getLoginUser().getUserId(),stateQuery, roleCode, searchCustomer, null, getPage());
		
/*		customerApplyList = officeCustomerApplyService.getCustomerApplyAuditList(customer, 
				getLoginUser().getUserId(),startTime,endTime,applyUserName,customerName, stateQuery, roleCode, searchCustomer, null, getPage());
...
*/		return SUCCESS;
	...
	public String auditEdit(){
		...
		Map<String,Dept>  deptMap=deptService.getDeptMap(customer.getUnitId());
		Map<String,User> userMap=userService.getUserMap(customer.getUnitId());
		User user=userMap.get(apply.getApplyUserId());
		if(user!=null){
			customer.setApplyUserName(user.getRealname());
			deptNameMap=officeCustomerApplyService.getDeptNameMap(deptNameMap, deptMap, user.getId(), user.getDeptid());
			if(MapUtils.isNotEmpty(deptNameMap)){
				customer.setDeptName(deptNameMap.get(user.getId()));
			}
		}
