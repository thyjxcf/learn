...
public class OfficeMsgReceivingServiceImpl implements OfficeMsgReceivingService{
	...
	public void changeState(String receiveUserId,String[] ids,int state) {
		...
		officeMsgReceivingDao.changeState(receiveUserId,ids,state);
	}
	...
	public Integer getNumber(String userId, String msgType, String readType) {
		return officeMsgReceivingDao.getNumber(userId, msgType, readType);
	};
	@Override
	public Integer getNumberForReceive(MessageSearch search, String msgType, String readType){
		return officeMsgReceivingDao.getNumberForReceive(search,msgType,readType);
	}
	@Override
	...
	public List<String> getIsReadInfoUserIds(String messageId, Integer isRead){
		return officeMsgReceivingDao.getIsReadInfo(messageId, isRead);
	}
	...
	public List<ReadInfoDto> getIsReadInfo(String messageId, Integer isRead) {
		...
		Map<String, Date> readDateMap = officeMsgReceivingDao.getReadDate(messageId);
		...
		for(Unit unit:unitMap.values()){
			...
			for(User user:users){
				if(unit.getId().equals(user.getUnitid())){
					...
					if (teacher != null) {
						...
						Date date = readDateMap.get(user.getId());
						String dateStr = DateUtils.date2String(date, "yyyy-MM-dd HH:mm");
						userDeptNames.add((dept == null ? "" : dept.getDeptname())
								+ "<span>"
								+ teacher.getName() + (StringUtils.isNotBlank(dateStr)?"("+dateStr+")":"")
								+ "</span>");
					}
			...
			for(Dept dept:depts){
				if(unit.getId().equals(dept.getUnitId())){
					Date date = readDateMap.get(dept.getId());
					String dateStr = DateUtils.date2String(date, "yyyy-MM-dd HH:mm");
					userDeptNames.add("<span>"+dept.getDeptname()+ (StringUtils.isNotBlank(dateStr)?"("+dateStr+")":"")+"</span>");
					detailNames.add(dept.getDeptname());
				}
			}
			//单位
			for(Unit unit1:units){
				if(unit.getId().equals(unit1.getId())){
					Date date = readDateMap.get(unit1.getId());
					String dateStr = DateUtils.date2String(date, "yyyy-MM-dd HH:mm");
					userDeptNames.add("<span>"+unit1.getName()+ (StringUtils.isNotBlank(dateStr)?"("+dateStr+")":"")+"</span>");
					detailNames.add(unit1.getName());
				}
			}
...
//		System.out.println("----列表查询结束----->"+DateUtils.date2String(new Date(), "yyyy-MM-dd HH:mm:ss"));
		...
		int i = 0;
		Set<String> sendUserIds=new HashSet<String>();
		for(OfficeMsgReceiving officeMsgReceiving:officeMsgReceivings){
			replyMsgIds[i] = officeMsgReceiving.getReplyMsgId();
			sendUserIds.add(officeMsgReceiving.getSendUserId());
			i++;
		}
		Map<String,String> photoUrlMap=userSetService.getUserPhotoMap(sendUserIds.toArray(new String[0]));//取头像
...
//		获取整个会话是否有加星的，如果有一个加星，那么外面显示加星
		...
		for(OfficeMsgReceiving officeMsgReceiving:officeMsgReceivings){
			...
			officeMsgReceiving.setPhotoUrl(photoUrlMap.get(officeMsgReceiving.getSendUserId()));
		}
...
//		获取整个会话是否有未读的，如果有一个未读，那么外面显示未读
	...
	public List<OfficeMsgReceiving> getOfficeMsgReceivingListByMessageIds(String[] messageIds){
		return officeMsgReceivingDao.getOfficeMsgReceivingListByMessageIds(messageIds);
	}
	@Override
	public List<OfficeMsgReceiving> getOfficeMsgReceivingListByReplyMsgIdForMobile(
			String replyMsgId, String userId,boolean deptRole,boolean unitRole) {
		...
		if(user!=null){
			if(deptRole){
				allIds.add(user.getDeptid());
			}
			if(unitRole){
				allIds.add(user.getUnitid());
			}
		}
		
		//获取为收件人的数据
		List<OfficeMsgReceiving> officeMsgReceivings = officeMsgReceivingDao.getOfficeMsgReceivingListByReplyMsgIdForMobile(replyMsgId, allIds.toArray(new String[0]));
	...
	@Override
	public List<OfficeMsgReceiving> getOfficeMsgDetails(String userId){//TODO
		List<OfficeMsgReceiving> list = new ArrayList<OfficeMsgReceiving>();
		User user = userService.getUser(userId);
		if(user == null){
			return list;
		}
		list = officeMsgReceivingDao.getOfficeMsgReceivings(userId);
		String unitid = user.getUnitid();
		if(customRoleService.isCustomRole(unitid, userId, "69", "unit_receiver")){
			Unit unit = unitService.getUnit(unitid);
			if(unit != null){
				List<OfficeMsgReceiving> ulist = officeMsgReceivingDao.getOfficeMsgReceivings(unitid);
				list.addAll(ulist);
			}
		}
		String deptid = user.getDeptid();
		if(StringUtils.isNotBlank(deptid) && customRoleService.isCustomRole(unitid, userId, "69", "dept_receiver")){
			Dept dept = deptService.getDept(deptid);
			if(dept != null){
				List<OfficeMsgReceiving> dlist = officeMsgReceivingDao.getOfficeMsgReceivings(deptid);
				list.addAll(dlist);
			}
		}
		
		Collections.sort(list, new Comparator<OfficeMsgReceiving>(){
			@Override
			public int compare(OfficeMsgReceiving o1, OfficeMsgReceiving o2){
	...
	public Map<String, Date> getReadDate(String messageId){
		return officeMsgReceivingDao.getReadDate(messageId);
	}
