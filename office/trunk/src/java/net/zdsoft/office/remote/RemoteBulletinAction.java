...
public class RemoteBulletinAction extends OfficeJsonBaseAction {
	...
	public void bulletinList() {
		...
		for(OfficeBulletin bulletin:officeBulletinList){
			...
			if(StringUtils.isNotBlank(bulletin.getTitle())&&bulletin.getTitle().length()>25){
	...
	public void bulletinDetail() {
		...
		if(StringUtils.isNotBlank(content)){
			//处理附件路径 解决ios端不能查看附件问题
			if(content.indexOf("/common/downloadFile.action?") > 0){
				content = content.replaceAll("/common/downloadFile.action\\?", "/common/downloadFile.action?isExternalLink=1&type=1&");
			}
		}
