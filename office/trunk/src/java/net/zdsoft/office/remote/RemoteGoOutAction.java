...
public class RemoteGoOutAction extends OfficeJsonBaseAction{
	...
	public void applyGoOut(){
		if(StringUtils.isNotEmpty(id)){
			...
			officeGoOut=officeGoOutService.getOfficeGoOutByOfficeLog(officeGoOut, unitId, userId);
		}
		if(CollectionUtils.isNotEmpty(officeGoOut.getFlowList())){
		...
		json.put("applyUserName", officeGoOut.getApplyUserName());
...
//			file = StorageFileUtils.handleFile(new String[] {}, 5*1024);
			...
			if(StringUtils.isNotEmpty(officeGoOut.getId())){
				...
				officeGoOutService.update(officeGoOut,uploadFileList,true,removeAttachmentArray);
			}else{
				officeGoOutService.add(officeGoOut,uploadFileList,true);
			}
			json.put("result",1);
		}catch (Exception e) {
			e.printStackTrace();
...
//			file = StorageFileUtils.handleFile(new String[] {}, 5*1024);
			...
			officeGoOutService.startFlow(officeGoOut,userId,uploadFileList,true, removeAttachmentArray);
...
//		page.setPageSize(15);
	...
	public void auditGoOut(){
		...
		json.put("hisTaskCommentArray",RemoteOfficeUtils.createHisTaskCommentArray(officeGoOut.getHisTaskList(), officeGoOut.getUnitId(), officeGoOut.getFlowId()));
	...
	public void auditPassGoOut(){
		...
		try {
			...
			officeGoOutService.passFlow(isPass(),taskHandlerSave,id,currentStepId);
			json.put("result",1);
		} catch (Exception e) {
			e.printStackTrace();
			json.put("result",0);
