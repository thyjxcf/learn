...
//		page.setPageSize(15);
	...
	public void applyBusinessTrip(){
		...
		json.put("applyUserName", officeBusinessTrip.getApplyUserName());
...
//			file = StorageFileUtils.handleFile(new String[] {}, 5*1024);
			...
			if(StringUtils.isNotEmpty(officeBusinessTrip.getId())){
				...
				officeBusinessTripService.update(officeBusinessTrip,uploadFileList,true,removeAttachmentArray);
			}else{
				officeBusinessTripService.add(officeBusinessTrip,uploadFileList,true);
			}
			jsonMap.put("result",1);
		}catch (Exception e) {
			e.printStackTrace();
...
//			file = StorageFileUtils.handleFile(new String[] {}, 5*1024);
			...
			officeBusinessTripService.startFlow(officeBusinessTrip,officeBusinessTrip.getApplyUserId(),uploadFileList,true, removeAttachmentArray);
			jsonMap.put("id",officeBusinessTrip.getId());
			jsonMap.put("result",1);
		}catch (Exception e) {
			e.printStackTrace();
			e.printStackTrace();
...
//		page.setPageSize(15);
		if(auditStatus==0){
			...
			officeBusinessTripList=officeBusinessTripService.HaveDoAudit(userId,true,page);
		}
		JSONArray array = new JSONArray();
		for(OfficeBusinessTrip officeBusinessTrip : officeBusinessTripList){
			JSONObject json = new JSONObject();
	...
	public void aduitBusinessTrip(){
		...
		json.put("hisTaskCommentArray",RemoteOfficeUtils.createHisTaskCommentArray(officeBusinessTrip.getHisTaskList(), officeBusinessTrip.getUnitId(), officeBusinessTrip.getFlowId()));
		jsonMap.put(getDetailObjectName(), json);
		jsonMap.put("result", 1);
		responseJSON(jsonMap);
	}
	public void auditPassBusinessTrip(){
		...
		try {
			...
			officeBusinessTripService.passFlow(isPass(),taskHandlerSave,id,currentStepId);
			json.put("result",1);
		} catch (Exception e) {
			e.printStackTrace();
			json.put("result",0);
