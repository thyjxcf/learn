...
<script>
	...
	if(flag){
		...
		attachmentUploader = WebUploader.create({
	        ...
	        resize: false,
	        // swf文件路径
	        swf: '${request.contextPath}/office/sealmanage/webuploader/Uploader.swf',
	        // 文件接收服务端。
	        server: '${request.contextPath}/office/sealmanage/sealmanage-saveFileAttach.action',
	        // 选择文件的按钮。可选。
	    	...
	    	if(data.operateSuccess){
				...
				if(data.businessValue){
				    ...
				    if('${extNames}'.indexOf(file.ext)!=-1){
			       		innerHtml += '<a href="javascript:void(0);" class="att_show" style="margin-left:4px;" dataId="'+returnValue[0]+'" onclick="viewAttachment(\''+returnValue[0]+'\',\''+file.ext+'\')">预览</a>';
			        }
				    innerHtml += '<a href="javascript:void(0);" style="margin-left:20px;" onclick="doDeleteAtt(\'\',\''+returnValue[0]+'\',\''+file.id+'\')">删除</a> ';       		
...
<#macro archiveWebuploaderEditViewer canEdit=true showAttachmentDivId='showAttDiv' editContentDivId='editContentDiv' isSend=true loadDiv=true>
                    ...
                    <li class="fn-clear">
                        ...
                        <div class="dd doc-list">
				            <ul class="fn-clear" id="attachmentP">
				                <#if officeSeal.attachments?exists>
									<#list officeSeal.attachments as att>
										<li id="attP${att_index}">
						                    ...
						                    <span id="${att.id}toSave">
						                		...
						                		<#if canEdit>
				                				<a href="javascript:void(0);" style="margin-left:10px;" onclick="doDeleteAtt('${att_index}','${att.id!}','')">删除</a>
				                				</#if>
						                    </span>
						                </li>
									</#list>
    ...
    <script>
		...
		function doDelete(num,attId,fileId,type,isContent,isNotClearHtml){
			...
			$.ajax({
				type: "POST",
				url: "${request.contextPath}/office/sealmanage/sealmanage-deleteFileAttach.action",
