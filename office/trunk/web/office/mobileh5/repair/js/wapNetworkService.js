var wapNetworkService = {
	...
	doLoadCount : function(data){
		if (this._service._doError(data)) return;
		wapNetworkService._service._loadListCount(data);
	},
	...
	doLoadAdmin : function(data){
		if (this._service._doError(data)) return;
		wapNetworkService._service._loadAdmin(data);
	},
...
//                		</li>
					...
					}else if(dataType == WapConstants.DATA_TYPE_1 || dataType == WapConstants.DATA_TYPE_2){
...
//	    		<li><span>事假<i class="icon-img icon-current"></i></span></li>
	    ...
	    _loadEdit : function(dataType, data){
    		...
    		$('#equipmentType').mobiscroll().select({
		        theme: 'mobiscroll',
		        lang: 'zh',
		        display: 'bottom',
		    });
    		...
    		$("#type").on('change',function(e){
        		...
        		$('#repaireTypeId').mobiscroll().select({
    		        theme: 'mobiscroll',
    		        lang: 'zh',
    		        display: 'bottom',
    		    });
	    ...
	    _loadAudit : function(dataType, data){
	    	...
	    	if(isNotBlank(result.repaireTypeId)){
	    		$("#repaireTypeId").html(result.repaireTypeId);
	    	}else{
	    		$("#repaireTypeId").parent("li").hide();
	    	}
