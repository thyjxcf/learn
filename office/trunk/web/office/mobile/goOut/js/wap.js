var wap = {
		init : function(contextPath, unitId, userId, userRealName){
			...
			location.href = contextPath + "/office/mobile/goOut/goOutList.html";
//			location.href = 'http://192.168.0.95/';
		},
...
//					
				...
				$("#apply").click(function(){
					var dataType = $('.tab-wrap ul').children('.current').val();
					location.href = "goOutApplyEdit.html?dataType="+dataType;
				});
			...
			backList:function(dataType){
				location.href="goOutList.html?dataType="+dataType+"&isBack="+WapConstants.BACK_LIST_GLAG_1;
			},
		...
		_editService : {
			...
			save : function(dataType){
				...
				if(!isNotBlank(goOutDays)){
					...
					if(!validateInteger(goOutDays)){
						viewToast('外出时间只能输入正整数');
						return;
					}
				}
			...
			initBind : function(){
				...
				$('#startDate').scroller($.extend(opt['dateYMDHI'],opt['default']));
				$('#endDate').scroller($.extend(opt['dateYMDHI'],opt['default']));
				$('#leaveFlow').scroller($.extend(opt['select'],opt['default']));			
				$('#goOutType').scroller($.extend(opt['select'],opt['default']));			
