...
//					mcodes = data.expenseTypeMcodes;
//					if(mcodes.length > 0){
//						storage.set(WapConstants.expense_TYPE_MCODES, JSON.stringify(mcodes));
//					}
//				}
					...
					$.each(array, function(index, json) {
						...
						if(WapConstants.DATA_TYPE_2 == dataType){//审核
							if(Constants.APPLY_STATUS_2 == state){//待审核
								htmlInfo = 'location.href=\'expenseAuditDetail.html?id='+json.id+'&dataType='+dataType+'&taskId='+json.taskId+'\'';
							}else{
								htmlInfo = 'location.href=\'expenseDetail.html?id='+json.id+'&dataType='+dataType+'\'';
							}
						}else{//申请
							if(Constants.APPLY_STATUS_1 == state){//未提交
								htmlInfo = 'location.href=\'expenseApplyEdit.html?id='+json.id+'&dataType='+dataType+'\'';
							}else{
								htmlInfo = 'location.href=\'expenseDetail.html?id='+json.id+'&dataType='+dataType+'\'';
							}
						}
                        ...
                        listHtmlStr += '<p class="tt f-18">'+json.detail+'</p>';
                        listHtmlStr += '<p class="dd fn-clear f-14">';
                        listHtmlStr += '<span class="dd-1">'+json.expenseType+'</span>';
                        if(WapConstants.DATA_TYPE_2 == dataType){
                        	listHtmlStr += '<span class="dd-3">'+json.applyUserName+'</span>';
                        }
                        listHtmlStr += '<span class="dd-2">'+json.createTime+'</span>';
                        listHtmlStr += '</p>';
                        listHtmlStr += '</li>';
					});
