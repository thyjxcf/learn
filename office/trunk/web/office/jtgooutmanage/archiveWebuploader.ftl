...
<script>	
	...
	attachmentUploader = WebUploader.create({
        ...
        resize: false,
        // swf文件路径
        swf: '${request.contextPath}/office/jtgooutmanage/webuploader/Uploader.swf',
        // 文件接收服务端。
        server: '${request.contextPath}/office/jtgooutmanage/jtgooutmanage-fileSave.action',
        // 选择文件的按钮。可选。
...
<#macro archiveWebuploaderEditViewer canEdit=true showAttachmentDivId='showAttDiv' editContentDivId='editContentDiv' isSend=true loadDiv=true>
                    ...
                    <li class="fn-clear">
                        ...
                        <div class="dd doc-list">
                        	<ul class="fn-clear" id="attachmentP">
                                <#if officeJtGoout.attachments?exists>
									<#list officeJtGoout.attachments as att>
    ...
    <script>
    	var divWidth = $("#editContentDiv").css("width");
		...
		function doDelete(num,attId,fileId,type,isContent,isNotClearHtml){
			...
			$.ajax({
				type: "POST",
				url: "${request.contextPath}/office/jtgooutmanage/jtgooutmanage-fileDelete.action",
