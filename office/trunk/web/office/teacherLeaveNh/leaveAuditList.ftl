    ...
    <#if teacherLeaveNhList?exists && teacherLeaveNhList?size gt 0>
    	<#list teacherLeaveNhList as leave>
    		<tr>
		    	...
		    	<td class="t-center">
		    		<#if leave.state==2>
	    				<a href="javascript:void(0);" onclick="auditLeaveApply('${leave.id!}','${leave.taskId!}');"><img src="${request.contextPath}/static/images/icon/edit.png" alt="编辑"></a>
		    		<#else>
...
<@common.Toolbar container="#leaveAuditListDiv"/>
</form>
<script>
	function auditLeaveApply(id,taskId){
		var url="${request.contextPath}/office/teacherLeaveNh/teacherLeaveNh-leaveAuditEdit.action?teacherLeaveNhId="+id+"&taskId="+taskId;
		load("#leaveAuditListDiv", url);
	}
	function viewInfo(id){
		load("#leaveAuditListDiv","${request.contextPath}/office/teacherLeaveNh/teacherLeaveNh-viewDetail.action?teacherLeaveNhId="+id);
	}
