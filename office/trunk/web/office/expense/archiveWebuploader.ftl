...
<script>
	...
	if(flag){
		...
		attachmentUploader = WebUploader.create({
	        ...
	        resize: false,
	        // swf文件路径
	        swf: '${request.contextPath}/office/expense/webuploader/Uploader.swf',
	        // 文件接收服务端。
	        server: '${request.contextPath}/office/expense/expense-saveFileAttach.action',
	        // 选择文件的按钮。可选。
...
<#macro archiveWebuploaderEditViewer canEdit=true showAttachmentDivId='showAttDiv' editContentDivId='editContentDiv' isSend=true loadDiv=true>
                    ...
                    <li class="fn-clear">
                        ...
                        <div class="dd doc-list">
				            <ul class="fn-clear" id="attachmentP">
				                <#if officeExpense.attachments?exists>
									<#list officeExpense.attachments as att>
    ...
    <script>
		...
		function doDelete(num,attId,fileId,type,isContent,isNotClearHtml){
			...
			$.ajax({
				type: "POST",
				url: "${request.contextPath}/office/expense/expense-deleteFileAttach.action",
