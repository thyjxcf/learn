...
<script>
	...
	if(flag){
		...
		attachmentUploader = WebUploader.create({
	        ...
	        resize: false,
	        // swf文件路径
	        swf: '${request.contextPath}/office/msgcenter/webuploader/Uploader.swf',
	        // 文件接收服务端。
	        server: '${request.contextPath}/office/msgcenter/msgcenter-saveFileAttach.action',
	        // 选择文件的按钮。可选。
				...
				if(data.businessValue){
				    ...
				    var name=filename.substring(filename.lastIndexOf(".")+1,filename.length); 
				       /* if(name.toUpperCase()=='GIF'||name.toUpperCase()=='JPG'||name.toUpperCase()=='JPEG'||name.toUpperCase()=='BMP'||name.toUpperCase()=='PNG'){
				        ...
				        innerHtml += '<a href="javascript:void(0);" class=""  onclick="doDeleteAtt(\'\',\''+returnValue[0]+'\',\''+file.id+'\',\'true\')">删除</a> ';       		
...
<#macro archiveWebuploaderEditViewer canEdit=true showAttachmentDivId='showAttDiv' editContentDivId='editContentDiv' isSend=true loadDiv=true>
				                ...
				                <#if officeMsgSending.attachments?exists>
									<#list officeMsgSending.attachments as att>
										<li id="attP${att_index}">
						                    ...
						                    <span class="attachmentSave" style="width:280px;float:left;" title="${att.fileName!}"><@common.cutOff str='${att.fileName!}' length=30/></span>
						                    <span id="${att.id}toSave">
						                        <a href="javascript:void(0);"  onclick="doDownload('${att.downloadPath!}');">下载</a>&nbsp;
				                				<a href="javascript:void(0);" onclick="doDeleteAtt('${att_index}','${att.id!}','','${canDeFile!}')">删除</a>
						                    </span>
						                </li>
									</#list>
								</#if>
    ...
    <script>
		...
		function doDeleteAtt(num,attId,fileId,canDeFile){
			 if(confirm("确认要删除该附件,该操作会删除服务器上的附件?")){
			 	doDelete(num,attId,fileId,0,0,canDeFile);
			 }
		}
		
		
		//删除附件
		function doDelete(num,attId,fileId,type,isContent,canDeFile,isNotClearHtml){
			if(isNotClearHtml){
				isNotClearHtml = true;
			}else{
			...
			$.ajax({
				type: "POST",
				url: "${request.contextPath}/office/msgcenter/msgcenter-deleteFileAttach.action?canDeFile="+canDeFile,
