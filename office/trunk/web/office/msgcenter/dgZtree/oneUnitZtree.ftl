...
<SCRIPT type="text/javascript">
	...
	function treeItemCheck(treeNode,cascade){
		if(cascade){
			var zTree = $.fn.zTree.getZTreeObj("currentUnitTree");
	...
	function searchNode(){
		var zTree = $.fn.zTree.getZTreeObj("currentUnitTree");
		var searchCurrentName = $("#searchCurrentName").val();
     	if (!searchCurrentName || searchCurrentName=="") {
     		init();
        	return;
      	}
		var nodeList = zTree.getNodesByParamFuzzy("name", searchCurrentName);
        ...
        var nodes = zTree.getNodesByFilter(function (node) {
        	if (node.name.indexOf(searchCurrentName)==-1) {
            	return true;
       		}
          	return false;
      	});
	...
	function init(){
		...
		$.fn.zTree.init($("#currentUnitTree"), setting, zNodes);
		//复选框 是否级联
		setCheckOption();
		$("#cascade").unbind("change").bind("change", setCheckOption);
		//搜索
		$("#searchCurrentName").unbind("keyup").bind("keyup", function(event){
...
<body>
<div class="search" style="white-space:nowrap;"><input type="text" style="width:80px;"  id="searchCurrentName" placeholder="输入搜索值"><span style="font-size: 12px">&nbsp;是否级联:</span><input type="checkbox" id="cascade" class="checkbox first" checked /></div>
<div class="ztree-inner"><ul id="currentUnitTree" class="ztree"></ul></div>
</body>
</html>