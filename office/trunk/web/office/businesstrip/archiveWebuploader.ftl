...
<script>
	...
	if(flag){
		...
		attachmentUploader = WebUploader.create({
	        ...
	        resize: false,
	        // swf文件路径
	        swf: '${request.contextPath}/office/businesstrip/webuploader/Uploader.swf',
	        // 文件接收服务端。
	        server: '${request.contextPath}/office/businesstrip/businessTrip-saveFileAttach.action',
	        // 选择文件的按钮。可选。
	        ...
	        formData :{
	        	businessTripId:'${officeBusinessTrip.id!}'
	        },
	    ...
	    attachmentUploader.on( 'uploadSuccess', function( file  ,data) {
	    	...
	    	if(data.operateSuccess){
				...
				if(data.businessValue){
				    ...
				    innerHtml += '<a href="javascript:void(0);" style="margin-left:10px;"  onclick="doDeleteAtt(\'\',\''+returnValue[0]+'\',\''+file.id+'\')">删除</a> ';       		
...
<#macro archiveWebuploaderEditViewer canEdit=true showAttachmentDivId='showAttDiv' editContentDivId='editContentDiv' isSend=true loadDiv=true>
    	...
    	<tr>
        	<td style="background:#fff;">
                <ul class="acc-wrap acc-wrap-dongguan">
                    <li class="fn-clear">
                    	<p class="tt"><span class="c-orange mr-5">*</span>附件</span>：</p>
                        <div class="dd doc-list">
				            <ul class="fn-clear" id="attachmentP">
				                <#if officeBusinessTrip.attachments?exists>
									<#list officeBusinessTrip.attachments as att>
    ...
    <script>
		...
		function doDelete(num,attId,fileId,type,isContent,isNotClearHtml){
			...
			$.ajax({
				type: "POST",
				url: "${request.contextPath}/office/businesstrip/businessTrip-deleteFileAttach.action",
