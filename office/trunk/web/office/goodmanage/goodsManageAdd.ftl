...
function doSave(){
	...
	var options = {
       url:'${request.contextPath}/office/goodmanage/goodmanage-goodsManage-save.action', 
       success : showReply,
       dataType : 'json',
       clearForm : false,
       resetForm : false,
...
<p class="tt"><a href="javascript:void(0);" class="close">关闭</a><span>物品登记</span></p>
...
<form id="addform" action="" method="post">
    <table border="0" cellspacing="0" cellpadding="0" class="table-form">
	    	...
	    	<td style="width:35%">
				...
				<div id="goodsNameFuzzyDiv" objectId="goodsName" class="search-box fn-left" style="z-index:1000;">
					<div>
						...
						<input type="text" value="${officeGoods.name!}" class="txt" onkeyup="delayLoadFuzzyDivWithData('goodsName',this.value);" onBlur="doHideFieldTip('goodsNameSelectedName');" id="goodsNameSelectedName">
							...
							<input type="hidden" name="goodsName" id="goodsName" value="${officeGoods.name!}"/>
					</div>
				</div>
	    ...
	    <tr>
	    	...
	    	<td style="width:35%">
	    		<input type="text" msgName="单价" class="input-txt fn-left" id="price" name="officeGoods.price" notNull="true" regex="/^(^[0-9]{0,5}$)|(^[0-9]{0,5}\.[0-9]{1,2}$)$/" regexMsg="最多5位整数2位小数" maxlength="10" value="${(officeGoods.price?default('0.00'))?string('0.00')}" style="width:175px;">
	    		<span class="c-orange mt-5 ml-10">*</span>
	    	</td>
	    </tr>
	    <tr>	
	    	...
	    	<td style="width:35%">
	    		<@htmlmacro.select style="width:185px;" valId="isReturnedStr" valName="officeGoods.isReturnedStr" >
        			<a val="0" <#if "0"==officeGoods.isReturnedStr!>class="selected"</#if>><span>需归还</span></a>
        			<a val="1" <#if "1"==officeGoods.isReturnedStr!>class="selected"</#if>><span>不需归还</span></a>
				</@htmlmacro.select>
	    		<span class="c-orange mt-5 ml-10">*</span>
	    	</td>
