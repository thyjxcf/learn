...
<#list objects as row>
<a id="${row.id!}" objectId="${idObjectId!}" href="javascript:void(0);" title="${row.objectName!}"><@common.cutOff str='${row.objectName!}' length=12/></a>
</#list>
<#else>
...
//树级菜单3	
	...
	$('.search-list-wrap a').unbind('click').click(function(e){
		e.stopPropagation();
		e.preventDefault();
		$(this).parent('.search-list-wrap').hide().siblings('.txt').val($(this).attr("id"));
