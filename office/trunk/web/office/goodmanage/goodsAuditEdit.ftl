...
function doSave(){
	...
	var options = {
       url:'${request.contextPath}/office/goodmanage/goodmanage-goodsAudit-save.action?officeGoodsReq.state=2', 
       success : showReply,
       dataType : 'json',
       clearForm : false,
       resetForm : false,
	...
	if(!data.operateSuccess){
			   if(data.errorMessage!=null&&data.errorMessage!=""){
		   		...
		   		showMsgSuccess("审核成功！","",function(){
		   			closeDiv("#goodsAuditLayer");
		   			doSearch();
				});
	}
}
</script>
<p class="tt"><a href="javascript:void(0);" class="close">关闭</a><span>领用审核</span></p>
<div class="wrap pa-10" id="contentDiv">
<form id="editform" action="" method="post">
    <table border="0" cellspacing="0" cellpadding="0" class="table-form">
    <input type="hidden" id="id" name="officeGoodsReq.id" value="${officeGoodsReq.id?default('')}">
	<tr>
    	<th class="pt-10" style="width:20%"><span class="c-orange mt-5 mr-5">*</span>审核意见：</th>
        <td class="pt-10" style="width:80%">
        	<textarea class="text-area my-5" id="advice" name="officeGoodsReq.advice" maxlength="1000" notNull="true" msgName="审核意见" style="width:90%;padding:5px 1%;height:50px;"></textarea>
        </td>
    </tr>
	</table>
</form>    
