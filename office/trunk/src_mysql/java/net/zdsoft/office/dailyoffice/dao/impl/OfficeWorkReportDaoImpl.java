...
public class OfficeWorkReportDaoImpl extends BaseDao<OfficeWorkReport> implements OfficeWorkReportDao{
	...
	public OfficeWorkReport save(OfficeWorkReport officeWorkReport){
		String sql = "insert into office_work_report(id, report_type, begin_time, end_time, receive_user_id, content, state, create_user_id, create_time, unit_id, dept_id,create_user_name,read_user_id) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		Object[] args = new Object[]{
			...
			officeWorkReport.getReadUserId()
		};
		update(sql, args);
		return officeWorkReport;
	}
	...
	public Integer update(OfficeWorkReport officeWorkReport){
		String sql = "update office_work_report set report_type = ?, begin_time = ?, end_time = ?, receive_user_id = ?, content = ?, state = ? ,create_time=sysdate where id = ?";
		Object[] args = new Object[]{
			officeWorkReport.getReportType(), officeWorkReport.getBeginTime(), 
			officeWorkReport.getEndTime(), officeWorkReport.getReceiveUserId(), 
			officeWorkReport.getContent(), officeWorkReport.getState(), 
			officeWorkReport.getId()
		};
	...
	public List<OfficeWorkReport> getOfficeWorkReportByUnitIdPageContent(
		...
		if(StringUtils.isNotBlank(beginTime)){
			sql.append(" and str_to_date(?, '%Y-%m-%d') <= str_to_date(date_format(begin_time,'%Y-%m-%d'), '%Y-%m-%d')");
			args.add(beginTime);
		}
		...
		sql.append(" order by case when state=1 then 1 when state=2 then 2 end, begin_time desc");
