	...
	public String saveCarApply(){
		...
		try {
			...
			if(officeCarApply.getState() == 2){
				...
				String deputyHeadId = dept.getDeputyHeadId();
				...
				for(CustomRoleUser ro : roleUserList){
					if(userMap.containsKey(ro.getUserId()) && !StringUtils.equals(ro.getUserId(), deputyHeadId)){
...
//						userName.append(userMap.get(ro.getUserId()).getName());
				...
				if(StringUtils.isNotBlank(deputyHeadId)){
					if(roleUser.length() > 0){
						roleUser.append(",");
					}
					roleUser.append(deputyHeadId);
				}
