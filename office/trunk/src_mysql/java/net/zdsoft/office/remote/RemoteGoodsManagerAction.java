		...
		try{
			...
			if(officeGoods == null){
				...
				if(officeGoods.getAmount() < applyAmount){
					...
					officeGoodsService.sendMsg(userService.getUser(userId), unitService.getUnit(unitId), officeGoods, null, null);
...
//				officeGoodsReqService.update(goodsReq);
				...
				officeGoodsService.sendMsg(user, unit, officeGoods, Integer.parseInt(applyType), new String[]{goodsReq.getGetUserid()});
