...
public class RemoteExpenseManageAction extends OfficeJsonBaseAction {
	...
	public void applyExpense() {//TODO
		...
		json.put("expenseMoney", officeExpense.getExpenseMoney());
	...
	public void submitExpenseApply(){
		...
		try {
			...
			officeExpenseService.startFlow(officeExpense, userId, file);
			json.put("result",1);
		}catch (Exception e) {
			e.printStackTrace();
			e.printStackTrace();
	...
	public void auditExpense() {//TODO
		...
		json.put("hisTaskCommentArray",RemoteOfficeUtils.createHisTaskCommentArray(officeExpense.getHisTaskList()));
	...
	public void saveExpenseAudit() {
		...
		try {
			...
			officeExpense = officeExpenseService.getOfficeExpenseById(id);
			officeExpenseService.doAudit(isPass(), taskHandlerSave, id);
			json.put("result",1);
		}catch (Exception e){
			e.printStackTrace();
			json.put("result",0);
