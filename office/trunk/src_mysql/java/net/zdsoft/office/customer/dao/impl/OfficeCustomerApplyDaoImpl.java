...
public class OfficeCustomerApplyDaoImpl extends BaseDao<OfficeCustomerApply> implements OfficeCustomerApplyDao{
	...
	public OfficeCustomerApply save(OfficeCustomerApply officeCustomerApply){
		String sql = "insert into office_customer_apply(id, unit_id, state,customer_id, follower_id, apply_type, deadline, delay_info, create_time,is_deleted,open_lecture) values(?,?,?,?,?,?,?,?,?,?,?)"; 
	...
	public Integer deletePutOffCustomer(String unitId,Date nowDate){//) or ( apply_type='2'and deadline <? and state=9) 
		String sql="update office_customer_info set state=2 where unit_id=? and progress_state!='06' and progress_state!='07' and progress_state!='08' and id in(select customer_id from office_customer_apply where  deadline <? and state=9 )";
		...
		update(sql1,new Object[]{true,unitId,nowDate});
		return update(sql,new Object[]{unitId,nowDate});
	}
	@Override
	...
	public List<OfficeCustomerApply> getAllList(SearchCustomer searchCustomer,String[] ids,String unitId,String userId,boolean cilent, Pagination page){
		StringBuilder sql=new StringBuilder("select a.* from office_customer_apply a inner join office_customer_info b on a.customer_id=b.id  ");
		List<Object> args=new ArrayList<Object>();
		sql.append(" where follower_id !=? and a.unit_id=? and a.state=9 and a.is_deleted=0 and  b.state=1 ");
		args.add(userId);
		args.add(unitId);
	...
	public List<OfficeCustomerApply> getOfficeCustomerApplyByUnitIdPage(String unitId,String userId,SearchCustomer searchCustomer, Pagination page){
		...
		sql.append(" inner join   base_flow_apply b  on a.id=business_id join office_customer_info c on c.id=a.customer_id where unit_id = ? and b.apply_user_id=? and apply_type!='2' and a.is_deleted=0 ");
	...
	public List<OfficeCustomerApply> getCustomerApplyAuditList(OfficeCustomerApply ent, String queryStatus, String roleId, int operateType, String[] arrangeIds, SearchCustomer customer, String[] applyTypes, Pagination page){
		...
		if(customer!= null && StringUtils.isNotBlank(customer.getRegion())){
			sql.append(" and info.region like ? ");
			args.add(customer.getRegion()+"%");
		}
		if(customer!= null && StringUtils.isNotBlank(customer.getType())){
