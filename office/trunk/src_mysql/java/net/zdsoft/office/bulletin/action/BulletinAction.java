...
public class BulletinAction extends PageAction {
	...
	public String manageList(){
		...
		officeBulletinList = officeBulletinService.getOfficeBulletinPage(getUnitId(),getLoginUser().getUserId(),userId,bulletinType,show,startTime,endTime, publishName, searName, searchAreaId, getPage());
	...
	public String bulletinPublishIds(){
		if(ArrayUtils.isEmpty(bulletinIds)){
			...
			try {
				...
				officeBulletinService.publish(idsSet.toArray(new String[0]),this.getLoginUser().getUserId());
			} catch (Exception e) {
				e.printStackTrace();
				jsonError = "发布失败";
			}
	...
	public String bulletinPublish(){
		if(StringUtils.isBlank(bulletinId)){
			...
			try {
				officeBulletinService.publish(new String[]{bulletinId},this.getLoginUser().getUserId());
			} catch (Exception e) {
				e.printStackTrace();
				jsonError = "发布失败";
			}
	...
	public String auditList(){
		...
		officeBulletinList = officeBulletinService.getOfficeBulletinPage(getUnitId(),getLoginUser().getUserId(),null,bulletinType,show,startTime,endTime, publishName, searName, searchAreaId, getPage());
