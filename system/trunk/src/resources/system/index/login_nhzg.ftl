...
<div id="container">
    <div class="login-wrap" <#if platFormSubsystem! =='fkxt'>style="background:url(${request.contextPath}/static/images/login-wrap_bg_fk.jpg) left top no-repeat #f6fbff;}"</#if>>
...
function login_() {
	...
	<#if connectPassport>
		<#-- login.js中会用到 -->
	  	...
	  	<#if deployPlatformEisUrl! !="">
	  		login.doLogin('${passportUrl}','${serverId}','${deployPlatformEisUrl}/fpf/homepage/subsystem.action?appId=62',<#if request.contextPath?default('') == ''>'1'<#else>'0'</#if>);
	  	<#else>
