<script>
function appSet(){
	openDiv('#commonApp',null,'${request.contextPath}/system/desktop/app/userApp-setEight.action',true,'#commonApp .wrap','476');
}

function addApp(){
	openDiv("#addSystem",null,"${request.contextPath}/system/desktop/app/externalApp-setEight.action");
}
...
<div class="dt fn-clear">
	...
	<#if userAppList?exists && 0 < userAppList?size>
	    <ul class="app-list">
	        <#list userAppList as app>
	        	<li><p><a href="javascript:void(0);" onClick="<#if app.picture! =="" || app.picture?index_of(".") != -1>go2ExternalApp('${app.url!}');<#else>go2Module('${app.url}','${app.subsystem}','${app.moduleId}','${app.parentId}','${app.name}','desktop','${app.limit!}');return false;</#if>"><img id="${app.moduleId}" src="<#if app.picture! =="" || app.picture?index_of(".") != -1>${request.contextPath}/static/images/ad/3.png<#else>${request.contextPath}${app.picture}_b.png</#if>">${app.name}</a></p></li>
	        </#list>
	    </ul>
	<#else>
     	<p class="no-data mt-5">还没有常用操作哦！</p>
    </#if>
</#if>
