		...
		var pushConnect = function(sourceId, targetIds) {
			...
			if(Zdsoft.app.g('operation') ==="process" ){//非编辑才控制
				...
				for(var i = 0; i < targetIds.length; i++) {
					...
					//保存条件信息
					if(sourceStep && sourceStep.stencil.id === "ExclusiveGateway"){
		...
		if(model && model.childShapes){
			...
			$.each(model.childShapes, function(i, item) {
				...
				if(d.id == Zdsoft.app.g('currentStepId')){
					...
					var curNextStep =that.instance.getStepMap(d.to[0].resourceId);
					if(curNextStep.stencil.id =="SequenceFlow"){
							var sequenceFlow=that.instance.getSequenceFlow(curNextStep.resourceId);
							var outgoing_resourceId =(sequenceFlow.outgoing)[0].resourceId;
							curNextStep=that.instance.getStepMap(outgoing_resourceId);
					}
					...
					if(stepType == "single"){
						...
						var d_next = {
							...
							taskUser:taskUser,
							taskUserId:taskUserId,
							top: (d.top+curNextStep.bounds.upperLeft.y)/2,
							left: (d.left+curNextStep.bounds.upperLeft.x)/2,
							name: stepName,
							permission:-1,
							to:d.to,
							step:step,
						...
						var now_step_array = [];
						var now_step_outgoing={resourceId:d_next.id};
						...
						var sequenceId =that.assembleSequence(sequence_index++,d.id,now_step_array);
						var before_sequence_array = [];
						var before_sequence_outgoing={resourceId:sequenceId};
						before_sequence_array.push(before_sequence_outgoing);
						_to=before_sequence_array;
						
					}else{
						var _index=++max;
						var id= 'ParallelGateway_'+_index;
						//先增加一个网关节点
						var d_before_gate ={
							id: id,
							index: _index,
							...
							step:'gate',
							cls:'wf-node wf-gate',
							stencil:{id:"ParallelGateway"},
							resourceId:id,
							properties:{permission:-1}
						};
						var before_gate_to_array = [];
						var before_gate_outgoing={resourceId:d_before_gate.id};
						before_gate_to_array.push(before_gate_outgoing);
						//放入map
						that.instance.putStepStateMap(d_before_gate.id,1);
						//将当前节点和本网关的sequenceId连接
						var sequenceId =that.assembleSequence(sequence_index++,d.id,before_gate_to_array);
						var current_step_sequence_array = [];
						var current_step_sequence_outgoing={resourceId:sequenceId};
						current_step_sequence_array.push(current_step_sequence_outgoing);
						_to=current_step_sequence_array;
						
						var taskUserIds=taskUserId.split(",");
						var taskUsers=taskUser.split(",");
						
						var before_gate_sequence_array=[];
						if(curNextStep.stencil.id  == "ParallelGateway1"){
							//如果下一步是网关节点
							for (var i = 0; i < taskUserIds.length; i++) {
								var _taskUserId =taskUserIds[i];
								var _taskUser=taskUsers[i];
								_index=++max;
								id= 'UserTask_'+step+"_"+_index;
								var d_next ={
									id: id,
									index: _index,
									type:'UserTask',
								...
								//将新加的task节点放入，后面会和前面的sequence连接
								var now_task_outgoing={resourceId:d_next.id};
								var now_task_array=[];
								now_task_array.push(now_task_outgoing);
								
								//存入map
								that.instance.putStepStateMap(d_next.id,"1");
								
								// 将新加的节点和后面的节点连接sequence
								//sequenceId=that.assembleSequence(sequence_index++,d_next.id, d.to);
								//var now_sequence_array=[];
								//var now_sequence_outgoing={resourceId:sequenceId};
								//now_sequence_array.push(next_sequence_outgoing);
								d_next.to=d.to;
								...
								//将新加的网关和本次新加的节点连接sequence
								sequenceId=that.assembleSequence(sequence_index++,d_before_gate.id, now_task_array);
								var before_gate_sequence_outgoing={resourceId:sequenceId};
								var temp_before_gate_sequence_array=[];
								before_gate_sequence_array.push(before_gate_sequence_outgoing);
								temp_before_gate_sequence_array.push(before_gate_sequence_outgoing);
								pushConnect(d_before_gate.id,temp_before_gate_sequence_array);
							}
		
							d_before_gate.to=before_gate_sequence_array;
							ret.steps.push(d_before_gate);
							that.instance.setStepMap(d_before_gate);
							
							
						}else{
							_index=++max;
							//如果下一步不是网关节点，需要增加一个网关节点
							id= 'ParallelGateway_'+_index;
							var d_next_gate ={
								id: id,
								index: _index,
								type:'ParallelGateway',
							...
							//存入map
							that.instance.putStepStateMap(d_next_gate.id,1);
							//sequenceId=that.assembleSequence(sequence_index++,d_next_gate.id, d.to);
							
							//var next_gate_sequence_outgoing={resourceId:sequenceId};
							//var next_gate_sequence_array=[];
							//next_gate_sequence_array.push(next_gate_sequence_outgoing);
							d_next_gate.to=d.to;
							that.instance.setStepMap(d_next_gate);
							ret.steps.push(d_next_gate);
							if(d_next_gate.to)
								pushConnect(d_next_gate.id, d_next_gate.to);
							
							var next_gate_outgoing={resourceId:d_next_gate.id};
							var next_gate_array=[];
							next_gate_array.push(next_gate_outgoing);
							...
							for (var i = 0; i < taskUserIds.length; i++) {
								...
								// 将新加的节点和后面的节点连接sequence
								sequenceId=that.assembleSequence(sequence_index++,d_next.id, next_gate_array);
								var now_sequence_array=[];
								var now_sequence_outgoing={resourceId:sequenceId};
								now_sequence_array.push(now_sequence_outgoing);
								
								d_next.to=now_sequence_array;
								ret.steps.push(d_next);
								that.instance.setStepMap(d_next);
								//存入map
								that.instance.putStepStateMap(d_next.id,"1");
...
var wfControll = {
	...
	editStep: function(id, op) {
		...
		if (id) {
			...
			eidtDialog = Ui.dialog({
				...
				top:10,
				init: function() {
					var param = {id:Zdsoft.app.g('id'),easyLevel:Zdsoft.app.g('easyLevel'),subsystemId:Zdsoft.app.g('subsystemId'),businessType:Zdsoft.app.g('businessType'),stepOperType:"edit",permission:selectStep.permission,taskUserId:selectStep.taskUserId,step:selectStep.step,stepName:selectStep.name};
	...
	addStep: function() {
		addDialog = Ui.dialog({
			...
			top:10,
			init: function() {
				var param = {id:Zdsoft.app.g('id'),easyLevel:Zdsoft.app.g('easyLevel'),subsystemId:Zdsoft.app.g('subsystemId'),businessType:Zdsoft.app.g('businessType'),stepOperType:"add",permission:0};
				var url= Zdsoft.app.url("jbmp/editor/wf-stepInfo.action",param);
				load("#addStepDiv",url);
			},
			ok: function() {
...
$(function() {
	...
	var wfInter = {
		...
		init: function(callback) {
			 var url = Zdsoft.app.url("jbmp/editor/wf-getData.action"),param = {id:id,subsystemId:subsystemId,instanceType:instanceType,currentStepId:currentStepId};
