...
webFXTreeConfig.loadingText = "正在载入...";
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon, sValue, sCheckbox) {
	// call super
	if(sCheckbox == "true"){
		webFXTreeConfig.needInput = "true";
		webFXTreeConfig.inputType = "checkbox";
		webFXTreeConfig.inputName = "idss";
	}
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon, sValue);
	...
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4) {
			if(xmlHttp.status == '200'){
				...
				//add by zhangza . 2006-11-7
				if(!jsNode.open) jsNode.expand();
			}else{
...
function _xmlTreeToJsTree(oNode) {
	...
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, openIcon, value, checkbox);
	else
		jsNode = new WebFXTreeItem(text, action, parent, icon, openIcon, value, checkbox);
...
function _xmlFileLoaded(oXml, jsParentNode) {
	...
	var oXmlDoc = XmlDocument.create();
	oXmlDoc.loadXML(oXml);
	// check that the load of the xml file went well
    if(oXmlDoc == null || oXmlDoc.documentElement == null) {
		/*
		jsParentNode.errorText = parseTemplateString(webFXTreeConfig.loadErrorTextTemplate,
							jsParentNode.src);
       alert("XML Parsing Error: " + oXmlDoc.parseError.reason);
	   */
       //return null;
    }
	else {
