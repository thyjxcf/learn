...
function loadAddressBookDetail(idObjectId,deptObjectId,deptLeaderObjectId,groupObjectId,nameObjectId,detailObjectId,height,currentUnit,url,params,changeHeight,callback,preset,needGroupId,currentUnitId){
	if (preset != null && preset != "") {
		if(eval(preset+"()") == false){
			return false;
		}
	}
	url=url+(url.indexOf('?') > -1 ? '&':'?')+"idObjectId="+idObjectId+"&deptObjectId="+deptObjectId+"&deptLeaderObjectId="+deptLeaderObjectId+"&groupObjectId="+groupObjectId+"&nameObjectId="+nameObjectId+"&detailObjectId="+detailObjectId+"&height="+height+"&currentUnit="+currentUnit+"&changeHeight="+changeHeight+"&callback="+callback+"&needGroupId="+needGroupId+"&currentUnitId="+currentUnitId;
	if(params != ""){
		url+="&"+params;
	} 	
...
function setNameListHtml(userSelection){
	...
	for (var index = 0, len = userSelection.length; index < len; ++index) {
		...
		if(item.type == 2){
			...
			nameStr=nameStr+"<li title="+item.detail+"><img src="+_contextPath+"/static/css/zTreeStyle/img/diy/04.png><a href=\"javascript:void(0);\" ondblclick=\"deleteObject('"+item.id+"');\">"+detailName+"</a><i onclick=\"deleteObject('"+item.id+"');\"></i></li>";
		}else if(item.type == 5){
			nameStr=nameStr+"<li title="+item.detail+"><img src="+_contextPath+"/static/css/zTreeStyle/img/diy/05.png><a href=\"javascript:void(0);\" ondblclick=\"deleteObject('"+item.id+"');\">"+detailName+"</a><i onclick=\"deleteObject('"+item.id+"');\"></i></li>";
		}
	}
	$("#userNameList").html(nameStr);
	$("#selectCount").html(userSelection.length);
	return false;
}
