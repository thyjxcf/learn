...
function setNameListHtml(userSelectionAll){
	...
	for (var index = 0, len = userSelectionAll.length; index < len; ++index) {
		...
		if(item.type == 1){
			nameStr=nameStr+"<li><img src="+_contextPath+"/static/css/zTreeStyle/img/diy/01.png><a href=\"javascript:void(0);\" ondblclick=\"removeTreeNode('"+item.id+"');\">"+detailName+"</a><i onclick=\"removeTreeNode('"+item.id+"');\"></i></li>";
		}else if(item.type == 2){
