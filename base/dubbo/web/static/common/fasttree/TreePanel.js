...
TreePanel.prototype={
	initialize : function(config){
		...
		//iconExt : {module:'../module.gif',depart:'../department.giff'}
		this.iconExt = config['iconExt'];
	...
	getIcon : function(icontype){//可以通过传入参数，覆盖默认图标。
		var icon_img = this.iconExt?this.iconExt[icontype]:undefined;
		if(icon_img == undefined){
			icon_img = this.icon[icontype];
		}
		return this.iconPath + icon_img;
	},
	...
	expand : function(node){
		//需要从顶层到下层，一层层调用expand()。
		if(node == null) return;
		var p;
		var expands = new Array();
		if(!node.childrenRendered) expands.push(node);
		p = node.parentNode;
		while(p && !p.childrenRendered){
			expands.push(p);
			p = p.parentNode;
		}
	...
	//左匹配查找,返回找到所有节点(遍历所有子节点)
	findChilds : function(attr,value){
		var nodes = [];
		for(var k in this.nodeHash){
			node = this.nodeHash[k];
			var str = node.attributes[attr].substr(0,value.length);
			if(str==value){
				nodes.push(node);
			}
		}
...
TreePanel.prototype.icon = {
	...
	female	: '../../xtree/images/female.gif',
	male	: '../../xtree/images/male.gif', 
	module	: '../../xtree/images/module.gif',
	dept	: '../../xtree/images/grade.gif',
	school	: '../../xtree/images/school.gif',
	unit	: '../../xtree/images/school.gif'
};
...
TreeNode.prototype={
	...
	collapse : function(deep){
		this.isExpand=false;
		if(this['html-element']['child'] && this['html-element']['child'].style){
			this['html-element']['child'].style.display='none';
		}
		this.paintIconImg();
		this.paintClipIcoImg();
	...
	select : function(){
		...
		this['html-element']['text'].style.background='highlight';
		this['html-element']['text'].style.color='highlighttext';
	},
	unselect : function(){
		this.isSelect = false;
		//this['html-element']['text'].style.backgroundColor='';
		this['html-element']['text'].style.background='';
