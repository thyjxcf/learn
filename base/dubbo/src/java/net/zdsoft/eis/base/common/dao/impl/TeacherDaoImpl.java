...
//			+ " where unit_id=? and institute_id=? and is_deleted=0 ORDER BY display_order";
	...
	public List<Teacher> getTeacherListByPolity(String unitId, String[] polities) {
		List<Teacher> teachers = Teacher.dt(teacherRemoteService.findByUnitId(unitId));
		List<Teacher> ts = new ArrayList<Teacher>();
		for(Teacher t : teachers){
			if(StringUtils.equals("21", t.getEusing()))
				continue;
			if(ArrayUtils.contains(polities, t.getPolity()))
				ts.add(t);
		}
		return ts;
...
//				" ORDER BY to_number(translate(display_order,'0123456789' || display_order,'0123456789'))");
	...
	public Teacher getTeacher(String teacherId) {
		Teacher teacher = Teacher.dc(teacherRemoteService.findById(teacherId));
		return teacher;
...
//				new String[] { teacherId }, new SingleRow());
	...
	public List<Teacher> getTeachers(String[] tchIds) {
		...
		return Teacher.dt(teacherRemoteService.findByIds(tchIds));
...
//				" ORDER BY to_number(translate(display_order,'0123456789' || display_order,'0123456789'))");
	...
	public List<Teacher> getTeachers(String unitId) {
		return Teacher.dt(teacherRemoteService.findByUnitId(unitId));
//		return query(SQL_FIND_TEACHER_BY_UNITID, unitId, new MultiRow());
	}

	public List<Teacher> getTeachersByDeptId(String deptId) {
		return Teacher.dt(teacherRemoteService.findByDeptId(deptId));
...
//				new MultiRow());
	...
	public List<Teacher> getExceptionTeachersByDeptId(String unitId, String deptId) {
		...
		List<Teacher> ts = new ArrayList<Teacher>();
		for(Teacher t : teachers){
			if(!StringUtils.equals(deptId, t.getDeptid())){
				ts.add(t);
			}
		}
		return ts;
...
//				new String[] { deptId }, new MultiRow());
	...
	public List<Teacher> getTeachersByFaintness(String unitId, String name) {
		return Teacher.dt(teacherRemoteService.findByNameCodeLike(name + "%", null, unitId));
...
//				name + "%" }, new MultiRow());
	...
	public List<Teacher> getTeachersByFaintness(String unitId,
		...
		return Teacher.dt(teacherRemoteService.findByNameCodeLike(teacherName, teacherCode, unitId));
//		if (null == teacherName) {
//			teacherName = "";
//		}
//		if (null == teacherCode) {
...
//				unitId, teacherName + "%", teacherCode + "%" }, new MultiRow());
	...
	public Map<String, Integer> getTeacherCount(String[] deptIds) {
		List<Teacher> ts = Teacher.dt(teacherRemoteService.findByDeptIds(deptIds));
		Map<String, Integer> rtnMap = new HashMap<String, Integer>();
		for(Teacher t : ts){
			Integer c = rtnMap.get(t.getDeptid());
			if(c == null)
				c = 0;
			c ++;
			rtnMap.put(t.getDeptid(), c);
		}
		return rtnMap;
...
//		return rtnMap;
	...
	public Map<String, Teacher> getTeacherMap(String unitId) {
		List<Teacher> teachers = Teacher.dt(teacherRemoteService.findByUnitId(unitId));
		return EntityUtils.getMap(teachers, "identityCardNo");
//		return queryForMap(SQL_FIND_TEACHER_BY_UNITID, new String[] { unitId },
//				new MapRow());
	}

	public Map<String, Teacher> getTeacherMap(String[] teacherIds) {
		List<Teacher> teachers = Teacher.dt(teacherRemoteService.findByIds(teacherIds));
		return EntityUtils.getMap(teachers, "id");
//		return queryForInSQL(
//				SQL_FIND_TEACHERS_BY_IDS,
//				null,
//				teacherIds,
...
//				" ORDER BY to_number(translate(display_order,'0123456789' || display_order,'0123456789'))");
	...
	public Map<String, Teacher> getTeacherWithDeletedMap(String[] teacherIds) {
		return getTeacherMap(teacherIds);
	}
//		return queryForInSQL(SQL_FIND_TEACHERS_BY_IDS_WITH_DELETED, null,
...
//	}
	...
	public List<Teacher> getTeacherByInstituteId(String unitId,
			String instituteId) {
		return Teacher.dt(teacherRemoteService.findByInstituteId(instituteId));
...
//				instituteId }, new MultiRow());
	...
	public List<Teacher> getTeachersOrder(String[] tchIds) {
		List<Teacher> ts = getTeachers(tchIds);
...
//				new MultiRow(), "order by f_trans_pinyin_capital(teacher_name)");
	...
	public boolean isExistPrincipan(String teacherId, String deptId) {
		Teacher teacher = Teacher.dc(teacherRemoteService.findById(teacherId));
		if(teacher == null || teacher.getIsdeleted() || !StringUtils.equals(teacher.getDeptid(), deptId)){
			return false;
		}
		return true;
...
//			objects.add("%"+searchName+"%");
	...
	public List<Teacher> getTeachersByDeptId(String deptId, Pagination page) {
		if (page != null) {
			return Teacher.dt(teacherRemoteService.findByDeptId(deptId, SUtils.s(page)), page);
//			return query(SQL_FIND_TEACHERS_BY_DEPT, new String[] { deptId },
//					new MultiRow(), page);
		} else {
			return Teacher.dt(teacherRemoteService.findByDeptId(deptId));
...
//					new MultiRow());
	...
	public Map<String, Teacher> getTeacherMapByUnitIdNameCodes(String unitId,
			String[] nameCodes) {
		List<Teacher> ts = getTeachers(unitId);
		Map<String, Teacher> map = new HashMap<String, Teacher>();
		for(Teacher t : ts){
			if(ArrayUtils.contains(nameCodes, t.getName()) || ArrayUtils.contains(nameCodes, t.getTchId())){
				map.put(t.getName() + t.getTchId(), t);
			}
		}
		return map;
...
//				});
	...
	public List<Teacher> getTeachersByUserState(String unitId) {
		return Teacher.dt(teacherRemoteService.findByUnitIdUserState(unitId, 1));
...
//		}
	...
	public List<Teacher> getTeachersByDeptIdOrUserState(String deptId) {
		return Teacher.dt(teacherRemoteService.findByDeptIdUserState(deptId, 1));
...
//		}
	...
	public List<Teacher> getTeachersByWeaveUnit(String weaveUnitId) {
		return Teacher.dt(teacherRemoteService.findByWeaveUnitId(weaveUnitId));
...
//		return query(sql, new Object[] { weaveUnitId }, new MultiRow());
	...
	public List<Teacher> findTeachersByUnitIds(String[] unitIds,
			String searchName) {
		return Teacher.dt(teacherRemoteService.findByNameCodeLike(searchName+ "%", null, unitIds));
...
//				+ searchName.trim() + "%" }, unitIds, new MultiRow());
	...
	public List<Teacher> getTeachersByDeptIdName(String deptId, String name) {
		List<Teacher> ts = Teacher.dt(teacherRemoteService.findByDeptId(deptId));
		List<Teacher> tarray = new ArrayList<Teacher>();
		for(Teacher t : ts){
			if(StringUtils.startsWith(t.getName(), name)){
				tarray.add(t);
			}
		}
		return tarray;
...
//				deptId, name + "%" }, new MultiRow());
	...
	public List<Teacher> getTeacherMapByIdentityCards(String[] identitycards) {
		return Teacher.dt(teacherRemoteService.findByIdentityCardNo(identitycards));
//		return queryForInSQL(
...
//				" ORDER BY to_number(translate(display_order,'0123456789' || display_order,'0123456789'))");
	...
	public List<Teacher> getTeachersByDeptIds(String[] deptIds) {
		return Teacher.dt(teacherRemoteService.findByDeptIds(deptIds));
//		return queryForInSQL(
...
//				" ORDER BY dept_id,to_number(translate(display_order,'0123456789' || display_order,'0123456789'))");
	...
	public List<Teacher> getTeachersByBirthday(String unitId,String beginDate, String endDate) {
		...
		List<Teacher> ts = new ArrayList<Teacher>();
		for(Teacher t : teachers){
			Date date = t.getBirthday();
			String dateStr = DateFormatUtils.format(date, "MMdd");
			if(dateStr.compareTo(beginDate) >= 0 && dateStr.compareTo(endDate) <= 0){
				ts.add(t);
			}
		}
		return ts;
...
//    }
	...
	public Map<String, Teacher> getTeacherByIdentityCards(String[] identitycards) {
		List<Teacher> teachers = SUtils.dt(teacherRemoteService.findByIdentityCardNo(identitycards), new TypeReference<List<Teacher>>(){});
		return EntityUtils.getMap(teachers, "idcard");
