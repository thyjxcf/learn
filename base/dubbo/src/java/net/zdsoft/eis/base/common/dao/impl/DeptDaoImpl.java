...
public class DeptDaoImpl extends BaseDao<Dept> implements DeptDao {
    ...
    public Dept getDept(String deptId) {
    	return Dept.dc(deptRemoteService.findById(deptId));
    	//SELECT * FROM base_dept WHERE is_deleted = 0 and id=?
        //return query(SQL_FIND_DEPT_BY_ID, deptId, new SingleRow());
    }
    
    @Override
    public Dept getDeptByName(String unitId, String deptName) {
    	List<Dept> list = Dept.dt(deptRemoteService.findByUnitId(unitId));
    	for(Dept d : list){
    		if(StringUtils.equals(deptName, d.getDeptname())){
    			return d;
    		}
    	}
    	return null;
    	//SELECT * FROM base_dept WHERE is_deleted = 0 and unit_id = ? and dept_name=?
    ...
    public Map<String, Dept> getDeptMap(String[] deptIds) {
    	List<Dept> list = Dept.dt(deptRemoteService.findByIds(deptIds));
    	return EntityUtils.getMap(list,"id");
    	//SELECT * FROM base_dept WHERE is_deleted=0 and id IN
    	//return queryForInSQL(SQL_FIND_DEPTS_BY_IDS, null, deptIds, new MapRow());
    }

    
    public List<Dept> getDepts(String unitId) {
    	return Dept.dt(deptRemoteService.findByUnitId(unitId));
        //SELECT * FROM base_dept where is_deleted=0 and unit_id = ? order by display_order
    	//return query(SQL_FIND_DEPTS_BY_UNITID, new String[] { unitId }, new MultiRow());
    }

    
    public List<Dept> getDeptsByParentId(String parentId) {
        return Dept.dt(deptRemoteService.findByParentId(parentId));
    	//return (List<Dept>) query(SQL_FIND_DEPTS_BY_PARENTID, new String[] { parentId },new MultiRow());
    }

    public List<Dept> getDirectDeptsByInstituteId(String instituteId) {
    	List<Dept> list = Dept.dt(deptRemoteService.findByInstituteId(instituteId));
    	List<Dept> lists = new ArrayList<Dept>();
    	for(Dept d : list){
    		if(StringUtils.equals(Dept.TOP_GROUP_GUID, d.getParentid())){
    			lists.add(d);
    		}
    	}
    	return lists;
        /*SELECT * FROM base_dept where is_deleted=0 and parent_id = ? and institute_id = ? order by display_order
    	return query(SQL_FIND_DEPTS_BY_PARENTID_INSTITUTEID, new Object[] { Dept.TOP_GROUP_GUID,instituteId }, new MultiRow());*/
    }
    
    public List<Dept> getDepts(String unitId, String parentId) {
    	return Dept.dt(deptRemoteService.findByUnitIdAndParentId(unitId,parentId));
    	//SELECT * FROM base_dept where unit_id = ? and parent_id = ? and is_deleted=0 order by display_order
        //return query(SQL_FIND_DEPTS_BY_UNITID_PARENTID, new String[] { unitId,parentId }, new MultiRow());
    }
    ...
    public List<Dept> getDirectDepts(String unitId) {
    	...
    	List<Dept> lists = new ArrayList<Dept>();
    	for(Dept d : list){
    		if(StringUtils.equals(Dept.TOP_GROUP_GUID, d.getParentid()) && StringUtils.equals(Dept.TOP_GROUP_GUID, d.getInstituteId())){
    			lists.add(d);
    		}
    	}
    	return lists;
       /* SELECT * FROM base_dept where unit_id = ? and parent_id = ? AND institute_id = ? and is_deleted=0 order by display_order
	...
	public List<Dept> getDeptsByParentId(String parentId, String deptName) {
		return Dept.dt(deptRemoteService.findByParentIdAndDeptName(parentId,deptName+"%"));
	...
	public List<Dept> getDepts(String unitId, String parentId, String deptName) {
		List<Dept> list = Dept.dt(deptRemoteService.findByUnitIdAndDeptNameLike(unitId, deptName));
		List<Dept> lists = new ArrayList<Dept>();
		for(Dept d : list){
			if(StringUtils.equals(parentId, d.getParentid())){
				lists.add(d);
			}
		}
		return lists;
	...
	public String isTeacherGroupHead(String teacherId) {
		List<Dept> list = Dept.dt(deptRemoteService.findByTeacherId(teacherId));
		List<Dept> dept = new ArrayList<Dept>();
		for(Dept d : list){
			if(StringUtils.equals(Dept.TOP_GROUP_GUID, d.getParentid()) && StringUtils.equals(Dept.TOP_GROUP_GUID, d.getInstituteId()) && d.getJymark() == 2){
				dept.add(d);
			}
		}
	...
	public String isPrincipanGroupHead(String userId) {
		List<Dept> dept = Dept.dt(deptRemoteService.findByTeacherId(userId));
	...
	public List<Dept> getDeputyHead(String unitId, String userId) {
		return Dept.dt(deptRemoteService.findByUnitIdAndDeputyHeadId(unitId,userId));
	...
	public List<Dept> getDeptsByAreaId(String areaId) {
		return Dept.dt(deptRemoteService.findByAreaId(areaId));
	...
	public List<Dept> getDeptList(String[] deptIds) {
		return Dept.dt(deptRemoteService.findByIds(deptIds));
	...
	public String[] getUserIdsByLeaderId(String unitId, String leaderId) {
		List<Dept> list = Dept.dt(deptRemoteService.findByUnitIdAndLeaderId(unitId,leaderId));
		List<String> listId = EntityUtils.getList(list,"teacherId");
