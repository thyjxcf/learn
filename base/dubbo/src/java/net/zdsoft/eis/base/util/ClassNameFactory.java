...
public class ClassNameFactory {
    ...
    public static ClassNameFactory getInstance() {
        ...
        if (null == basicClassService) {
            basicClassService = (BasicClassService) ContainerManager.getComponent("basicClassService");
        }
        if (null == semesterService) {
            semesterService = (SemesterService) ContainerManager.getComponent("semesterService");
        }
        if (null == gradeService) {
            gradeService = (GradeService) ContainerManager.getComponent("gradeService");
        }
        if (null == gradeDao) {
            gradeDao = (GradeDao) ContainerManager.getComponent("gradeDao");
        }
        if (null == mcodedetailService) {
            mcodedetailService = (McodedetailService) ContainerManager.getComponent("mcodedetailService");
        }
        return me;
    }
    ...
    public String getClassNameDyn(String classId) {
        ...
        return getClassNameDyn(cls.getSchid(), cls.getAcadyear(), cls.getSection(), cls.getSchoolinglen(),
                cls.getClassname());
    }
    ...
    public String getClassNameDyn(BasicClass cls, String curAcadyear) {
        ...
        return getClassNameDyn(cls.getAcadyear(), cls.getSection(), cls.getSchoolinglen(), cls.getClassname(),
                curAcadyear);
    }
    ...
    public String getClassNameDyn(String schoolId, String enrollyear, int section, int schoolinglen, String className) {
        ...
        if (grade != null && StringUtils.isNotBlank(grade.getGradename())) {
            return grade.getGradename() + (StringUtils.isBlank(className)?"":className);
        }
        return this.getClassNameDyn(enrollyear, section, schoolinglen, className, curacadyear);
    }

    private String getClassNameDyn(String enrollyear, int section, int schoolinglen, String className,
            String curAcadyear) {
        if (null == className) {
            className = "";
        }
        return getGradeNameDyn(enrollyear, section, schoolinglen, curAcadyear) + className;
    }
    ...
    public String getGradeNameDyn(String schid, int section, int grade) {
        ...
        if (StringUtils.isBlank(schid)) {
            return getGradeNameDyn(section, grade, getEnrollyear(curacadyear, grade));
        }
        Grade mygrade = gradeDao.getGrade(schid, getEnrollyear(curacadyear, grade), section);
    ...
    private static String getEnrollyear(String acadyear, int grade) {
        if (org.apache.commons.lang.StringUtils.isBlank(acadyear) || acadyear.length() != 9 || grade < 1)
            return "";

        String tempStartYear = acadyear.substring(0, 4);
        String tempEndYear = acadyear.substring(5);
        String startYear = String.valueOf(Integer.parseInt(tempStartYear) - (grade - 1));
        String endYear = String.valueOf(Integer.parseInt(tempEndYear) - (grade - 1));
    ...
    public String getGradeNameDyn(int section, int grade, String enrollyear) {
        ...
        if ("1".equals(systemIniService.getSystemIni(INIID).getNowValue())) {
            if (StringUtils.isBlank(NameFactory.yearMap.get(new Integer(grade)))) {
                gradeName = "异常年级-" + section + grade;
            }
            else {
                gradeName = sectionName + (String) NameFactory.yearMap.get(new Integer(grade));
            }
        }
        else {
            // 规则2: 2006级小 2006级初
    ...
    public String getGradeNameDyn(Grade grade) {
        return getGradeNameDyn(grade.getSchid(), grade.getAcadyear(), grade.getSection(), -1);
    }

    public String getGradeNameDyn(String schoolId, String enrollyear, int section, int schoolingLen) {
        String curacadyear = semesterService.getCurrentAcadyear();
    ...
    public String getGradeNameDyn(Grade grade, String curAcadyear) {
        return getGradeNameDyn(grade.getAcadyear(), grade.getSection(), grade.getSchoolinglen(), curAcadyear);
    }
     ...
     * @return String
     */
    public String getGradeNameDyn(String enrollyear, int section, int schoolinglen, String curAcadyear) {
        if (curAcadyear == null) {
            return "(非法入学学年)";
        }

        int grade = 1 + NumberUtils.toInt(StringUtils.substringBefore(curAcadyear, "-"))
                - NumberUtils.toInt(StringUtils.substringBefore(enrollyear, "-"));
        if (grade <= 0) {
            return "(非法入学学年)";
        }
        ...
        Map<String, Mcodedetail> detailMap = mcodedetailService.getMcodeDetailMap("DM-RKXD-" + section);
        ...
        gradeNameDyn = StringUtils.replace(gradeNameDyn, "{enrollyear}", StringUtils.substringBefore(enrollyear, "-"));
        return gradeNameDyn;
    }
    ...
    public void setGradeDyn(Grade grade) {
        if(grade == null)
            return;
        String curAcadyear = semesterService.getCurrentAcadyear();
        Grade mygrade = gradeDao.getGrade(grade.getSchid(), grade.getAcadyear(), grade.getSection());
        String gradeName = null;
        if (mygrade != null && StringUtils.isNotBlank(mygrade.getGradename())) {
            gradeName = mygrade.getGradename();
        }
        else {
            gradeName = getGradeNameDyn(grade.getAcadyear(), grade.getSection(), grade.getSchoolinglen(), curAcadyear);
        }
    ...
    public void setGradeDyn(Grade grade, String curAcadyear) {
        ...
        grade.setGradename(getGradeNameDyn(grade.getAcadyear(), grade.getSection(), grade.getSchoolinglen(),
                curAcadyear));
        grade.setGradeCodeDyn(curAcadyear);
    }
     ...
     * @return
     */
    public List<BasicClass> buildClassDyn(String schoolId, List<BasicClass> classList) {
        String curAcadyear = semesterService.getCurrentAcadyear();
        return buildClassDyn(classList, curAcadyear);
    }

    public List<BasicClass> buildClassDyn(List<BasicClass> classList, String curAcadyear) {
     ...
     * @return List
     */
    public List<String[]> buildClassNameDyn(String schoolId, List<BasicClass> classList) {
     ...
     */
    public Map<String, String> buildClassNameDynMap(String schoolId, List<BasicClass> classList) {
        String curAcadyear = semesterService.getCurrentAcadyear();
    ...
    public List<Grade> buildGradeDyn(String schoolId, List<Grade> gradeList, String curAcadyear) {
        if (null == curAcadyear) {
    ...
    // =====================年级Map====================
    public Map<String, Grade> buildGradeDynMap(String schoolId, List<Grade> gradeList) {
        String curAcadyear = semesterService.getCurrentAcadyear();
    ...
    public static void setMcodedetailService(McodedetailService mcodedetailService) {
