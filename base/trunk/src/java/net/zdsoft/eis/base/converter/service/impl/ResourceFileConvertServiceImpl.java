...
public class ResourceFileConvertServiceImpl implements BusinessTaskService {   
    ...
    private WindowsConverterFileService windowsConverterFileService;
    
    public void setWindowsConverterFileService(
			WindowsConverterFileService windowsConverterFileService) {
		this.windowsConverterFileService = windowsConverterFileService;
	}
    ...
    public void saveHandleBusinessTask(BusinessTask job, Reply reply) throws Exception {
        ...
        System.out.println("需要转换文件的路径------------"+sourcePath);
        int isNeedBack = 0;
        if (StringUtils.isNotEmpty(sourcePath)) {
            ...
            if (converterFileTypeService.isDocument(fileType)) {
                //TODO
            	if("1".equals(getConvertType())){
        			boolean isOk = windowsConverterFileService.conver(task);
        			if(isOk){
        				isNeedBack = 1;
        			}else{
        				isNeedBack = 2;
        ...
        // 更新记录
        if(isNeedBack == 1 ){
        	task.setStatus(BusinessTask.TASK_STATUS_WINDOW_HAND);
        	task.setResultMsg("window转换服务转换中");
        } else if(isNeedBack == 2 ){
        	task.setStatus(BusinessTask.TASK_STATUS_NO_HAND);
        	task.setResultMsg("window转换服务遇到问题，等待重新转换");
        }else{
	...
	private String getConvertType(){
		String type =  systemIniService.getValue(BaseConstant.SYSTEM_CONVERTOR_TYPE);
