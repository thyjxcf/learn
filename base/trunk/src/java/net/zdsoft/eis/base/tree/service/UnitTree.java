...
public class UnitTree extends XTreeMaker {
	...
	protected String composeZTree(String contextPath, String unitId)
		...
		SystemIni systemIni = systemIniService.getSystemIni("MSG.IN.SYSTEM");
		if(systemIni != null && "1".equals(systemIni.getNowValue())){
			isChange=true;
		}else{
			isChange=false;
		}
		List<Unit> edus = Lists.newArrayList();
		
		if(isChange){
			List<Unit> schs = Lists.newArrayList();
			Map<String,Unit> schUnitMap = Maps.newHashMap();
			Set<String> schoolIdSet=new HashSet<String>();
			for (Unit unit : list) {
				if(StringUtils.equals(unit.getUnitclass()+"", Unit.UNIT_CLASS_SCHOOL+"")){
					schoolIdSet.add(unit.getId());
					schUnitMap.put(unit.getId(), unit);
				}else{
					edus.add(unit);
				}
			...
			Map<String,School> schoolMap = schoolService.getSchoolsById(schoolIdSet.toArray(new String[0]));
			List<School> schools = Lists.newArrayList();
			if(schoolMap != null){
				schools = Lists.newArrayList(schoolMap.values());
			}
			Collections.sort(schools,new Comparator<School>() {
				@Override
				public int compare(School o1, School o2) {
					String a1=StringUtils.isBlank(o1.getSections())?"0":o1.getSections();
					String a2=StringUtils.isBlank(o2.getSections())?"0":o2.getSections();
					return a1.compareTo(a2);
				}
			});
			
			for (School school: schools) {
				Unit unit = schUnitMap.get(school.getId());
				schs.add(unit);
			}
			edus.addAll(schs);
		}else{
