...
public class RemoteCallUtils {
    ...
    public static <T> JSONObject convertJson(T o) {
      ...
      * @return
      */
    public static <T> JSONObject convertJsonMsg(T o, String msg) {
        JSONObject json = new JSONObject();
        if (o instanceof String) {
            json.put(JSON_RESULT_STR, o.toString());
            json.put(JSON_RESULT_STATUS, 1);
    ...
    /**
     * 获取参数(此方法客户端走openapi时调用,action中获取值请调用RemoteBaseAction中的getParamValue)
    ...
    public static String getParamValue(String remoteParam, String name){
    	...
    	JSONObject remoteParamJson = JSONObject.fromObject(decode(remoteParam));
     ...
     * @return
     */
    public static JSONObject getParamJson(String remoteParam){
    	JSONObject jsonParam = null ;
    	if(StringUtils.isBlank(remoteParam))
    		return jsonParam;
     ...
     * @return
     */
    public static String returnResultJsonMsg(Object os, String msg){
    	return encode(convertJsonMsg(os, msg).toString());
    }
    
    /**
     * 将一个数组，或者集合对象转换为json格式并加密
     * @param os
     * @return
     */
    public static String returnResultJsons(Object os){
    	return encode(convertJsons(os).toString());
    }
