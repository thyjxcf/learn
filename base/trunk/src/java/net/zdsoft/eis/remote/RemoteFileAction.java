...
public class RemoteFileAction extends BaseAction {
    ...
    public void remoteReceiveEncodeFile() throws Exception {
    	fileName = RemoteCallUtils.getParamValue(remoteParam, "fileName");
    	dirPath = RemoteCallUtils.getParamValue(remoteParam, "dirPath");
    	
    	try {
    		if(StringUtils.isBlank(fileName)){
	    		ServletUtils.print(getResponse(), RemoteCallUtils.returnResultError("fileName不能为空！"));
	            return;
    		}
    		
    		if(StringUtils.isBlank(dirPath)){
	    		ServletUtils.print(getResponse(), RemoteCallUtils.returnResultError("dirPath不能为空！"));
	            return;
    		}
            ...
            if (request instanceof MultiPartRequestWrapper) {
                StorageDir dir = storageDirService.getActiveStorageDir();
                if (dir == null) {
                    ServletUtils.print(getResponse(), RemoteCallUtils.returnResultError("没有设置Store目录！"));
                    return;
                }
                MultiPartRequestWrapper requestWrapper = (MultiPartRequestWrapper) request;
                Enumeration<String> e = requestWrapper.getFileParameterNames();
            ...
            JSONObject json = new JSONObject();
            json.put("message", "上传成功！");
            ServletUtils.print(getResponse(), RemoteCallUtils.returnResultJson(json));
		} catch (Exception e) {
