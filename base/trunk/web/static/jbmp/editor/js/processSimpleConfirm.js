...
//					permission = currentStep.properties.permission;
		...
		var getSameParallelSteps = function(stepId, steps) {
			var curNextStep = that.instance.getStepMap(stepId);
			var sequenceFlow = that.instance.getSequenceFlow(curNextStep.outgoing[0].resourceId);
			var outgoing_resourceId = (sequenceFlow.outgoing)[0].resourceId;
			var nextStep = that.instance
					.getStepMap(outgoing_resourceId);
			if(nextStep.stencil.id != "ParallelGateway"){
				steps.push(outgoing_resourceId);
				getSameParallelSteps(outgoing_resourceId,steps);
			}
		};
...
//							}
							parallelHeights[parallelCount++]=item.bounds.upperLeft.y;
...
//				inParallelGateway=true;
			...
			for(var i=0;i<parallelCount/2;i++){
				if (currentStepTop > parallelHeights[i*2]
						&& currentStepTop < parallelHeights[i*2+1]){
					beforeParallelGatewayHeight= parallelHeights[i*2];
					nextParallelGatewayHeight=parallelHeights[i*2+1];
					inParallelGateway = true;
				}
			}
			var sameParallelSteps=[];
			if(inParallelGateway){
				getSameParallelSteps(currentStepId,sameParallelSteps)
			}
			$.each(model.childShapes, function(i, item) {
				var _id = item.resourceId;
				if (item.stencil.id == "SequenceFlow") {
				...
				if(inParallelGateway){
					if($.inArray(item.resourceId, sameParallelSteps)!= -1){
						realTop = realTop + 100;
					}else{
						if(currentStepTop + 100*(sameParallelSteps.length+1) >= nextParallelGatewayHeight){
							if(item.bounds.upperLeft.y >= nextParallelGatewayHeight){
								realTop = realTop + 100;
							}
						}
					}
