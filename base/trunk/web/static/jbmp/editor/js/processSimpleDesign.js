...
//									}
									parallelHeights[parallelCount++]=stepDataList[id].top;
								}
							}
							var inParallelGateway = false;
							parallelHeights.sort(function numAscSort(a,b){return a - b;})
							for(var i=0;i<parallelCount/2;i++){
								if (stepDataList[index].top > parallelHeights[i*2]
										&& stepDataList[index].top < parallelHeights[i*2+1])
							...
							if (stepDataList[before_step_index].type == "ParallelGateway"
								...
								if (parallelNum == 1) {
									...
									for (var id in stepDataList) {
										...
										var selectedStep = $("#"
												+ stepDataList[id].id);
										if(parallelId ==stepDataList[id].id){
		...
		for (var ii = 0; ii < users.length; ii++) {
			...
			if (isParallelGateway) {
				...
				if (stepDataList[aboveId].type != "ParallelGateway") {
					...
					parallelGateIds.push(gate_before[0].id);
					...
					parallelGateIds.push(gate_after[0].id);
			...
			for (var id in stepDataList) {
				...
				if (top < stepDataList[id].top) {
					...
					if ($.inArray(stepDataList[id].id, parallelStepsIds) != -1)
						continue;
					if($.inArray(stepDataList[id].id, parallelGateIds) != -1){
						continue;
					}
//					if (stepDataList[id].type == "ParallelGateway"
...
//						continue;
			...
			top = $steps.position().top+$("#wf_designer_canvas").scrollTop();
...
//						nextParallelGatewayHeight = stepDataList[id].top;
//					}
					parallelHeights[parallelNum++]=stepDataList[id].top;
				}
				// 并行的步骤 有交叉的认定为并行步骤
				if (((top - stepDataList[id].top) > -ui.helper.outerHeight())
						&& ((top - stepDataList[id].top) < selectedStep
					...
					if (top - stepDataList[id].top > 0) {
						if (stepDataList[id].top > aboveTop) {
							aboveId = id;
							aboveTop = stepDataList[id].top;
							aboveStepType=stepDataList[id].type;
						}
					} else {
						// 下面的元素
						if (top < stepDataList[id].top) {
							if (stepDataList[id].top < belowTop) {
								belowId = id;
								belowTop = stepDataList[id].top;
								nextStepType=stepDataList[id].type;
							}
						}
					}
				}
			...
			var inParallelGateway = false;
			parallelHeights.sort(numAscSort)
			for(var i=0;i<parallelNum/2;i++){
				if (top > parallelHeights[i*2]
						&& top < parallelHeights[i*2+1]){
					inParallelGateway = true;
					beforeParallelGatewayHeight=parallelHeights[i*2];
					nextParallelGatewayHeight=parallelHeights[i*2+1];
				}
			}
			var multiParallel=false;
			for (var id in stepDataList) {
				if (stepDataList[id].top>beforeParallelGatewayHeight && stepDataList[id].top<nextParallelGatewayHeight) {
					if(stepDataList[id].top != parallelHeight){
						multiParallel=true;
						break;
					}
				}
			...
			// 并行步骤内部不支持多并行步骤
			if (inParallelGateway && multiParallel) {
				Ui.tip(U.lang('WF.INVALID_OPERATION_MORE_PARALLEL'),
						"warning");
				return;
			}
			// 多重网关
...
//			}
			parallel = true;
			if(!inParallelGateway &&("ParallelGateway" == nextStepType || "ParallelGateway" == aboveStepType)){
				if(isParallelGateway){
					Ui.tip(U.lang('WF.INVALID_OPERATION'), "warning");
					return;
				}
