        ...
        try {            
            ...
            if(inParam.isHasTitle()){
	            ...
	            if(requiredFileVersion != null) {
	                String fileVersion = StringUtils.substringAfterLast(objectDefine, "_");
	                if(!requiredFileVersion.equalsIgnoreCase(fileVersion)) {
	                    throw new Exception("导入文件版本["+fileVersion+"]与系统要求的版本["+requiredFileVersion+"]不匹配。");
	                }
	                objectDefine = StringUtils.substringBeforeLast(objectDefine, "_");
	            }  
