/* 
 * @(#)XMLConfigReader.java    Created on May 12, 2006
 * Copyright (c) 2005 ZDSoft Networks, Inc. All rights reserved.
 * $Header: f:/44cvsroot/stusys/OA3.6/java/src/net/zdsoft/fpf/server/util/xml/XMLConfigReader.java,v 1.1 2006/11/09 12:53:40 zhaosf Exp $
 */
package net.zdsoft.leadin.xml;
 ...
 * <p>
 * You instantiate the XMLConfigReader class in a program to read data from a
 * XML file. Once the class is instantiated, it reads all the data in a XML file
 * and loads into a DOM tree. Then it populates a hashtable so the getString
 * method can be called to find text information based on a key and subkey.
 * </p>
 * 
 ...
 * href="http://xmlconfigreader.sourceforge.net"
 * target="_blank">http://xmlconfigreader.sourceforge.net</a><br/> License:
...
public class XMLConfigReader extends ResourceBundle {
	 ...
	 */
	protected Hashtable<String, Hashtable<String, String>> hashcontents = null;
	 ...
	 * 
	 * @param xmlfile
	 *            the XML file to read, supports also URI resources or JAR
	 *            resources
	 */
	public XMLConfigReader(String xmlfile, String itemName) {
		...
		if (document == null) {
			hashcontents = new Hashtable<String, Hashtable<String, String>>(); // initialize a void hashtable
			return;
		}

		// Make a list of Items and count the number of items
		NodeList listOfItems = document.getElementsByTagName(itemName);
		...
		// set hash size
		hashcontents = new Hashtable<String, Hashtable<String, String>>(numberOfItems);
		...
		for (int i = 0; i < numberOfItems; i++) {
			...
			Hashtable<String, String> hashsubcontents = new Hashtable<String, String>(numberOfSUBs);
			...
			for (int j = 0; j < numberOfSUBs; j++) {
				...
				if (listOfSUBs.item(j).getNodeType() == Node.ELEMENT_NODE) {
					...
					temp_name = listOfSUBs.item(j).getNodeName(); // get
					// element
					// name
					try {
						temp_value = listOfSUBs.item(j).getFirstChild()
								.getNodeValue(); // get element value
					} catch (Exception e) {
						// System.err.println(this.getClass().getName() + "(\""
						// + xmlfile + "\") :: " + ":: Void <" + temp_name + ">
						// value on <" + temp_key + "> item");
						temp_value = "";
					}
					if ((temp_name != null) && (temp_value != null)) {
						hashsubcontents.put(temp_name, temp_value); // store
						// this
						// value on
						// hash
						// table
	...
	private Document parseXmlFile(String filename, boolean validating) {
		...
		try {
			try {
				...
				// InputStream instream = (new File(filename)).
				InputStream instream = ConfigFileUtils.getConfigFileStream(filename);
				...
				try {
					...
					try {
						...
						try {
							...
							String[] classPath = System.getProperties()
									.getProperty("java.class.path", ".").split(
											";");
    ...
    public String getString(String key, String subkey, String def) {
		...
		try {
			...
			Hashtable<String, String> subtable = (Hashtable<String, String>) this.getObject(key);
	...
	public String getKey(String subkey, String value) {
		...
		Enumeration<String> data_keys = getKeys(); // get current keys
		while (data_keys.hasMoreElements()) {
			current_key = data_keys.nextElement();
	...
	public static void main(String[] args) {
		XMLConfigReader res = new XMLConfigReader(
				"configfiles/application.xml", "app");
		Enumeration<String> enumeration = res.getKeys();
		while (enumeration.hasMoreElements()) {
			String key = enumeration.nextElement();
			System.out.println(res.getString(key, "name"));
			System.out.println(res.getString(key, "description"));
			System.out.println(res.getString(key, "value"));
		}
	}
}
